...
using cppbor::Uint;
using se_transport::SocketTransport;

// static function declarations
static uint16_t getApduStatus(std::vector<uint8_t>& inputData);
static int sendData(std::shared_ptr<SocketTransport>& pSocket, std::string input,
...
static inline std::shared_ptr<SocketTransport> getInstance() {
    static std::shared_ptr<SocketTransport> socket =
        std::shared_ptr<SocketTransport>(new SocketTransport());
    return socket;
}

// Print usage.
void usage() {
    printf("Usage: Please consturcture the apdu(s) with help of construct apdu tool and pass the "
           "output file to this utility.\n");
    printf("provision [options]\n");
    printf("Valid options are:\n");
    printf("-h, --help    show this help message and exit.\n");
    printf("-v, --km_version version \t Version of the keymaster(4.0 or 4.1 for keymaster; 100 for "
           "keymint \n");
...
static int sendData(std::shared_ptr<SocketTransport>& pSocket, std::string input,
    ...
    if (!pSocket->sendData(apdu.data(), apdu.size(), response)) {
...
int provisionData(std::shared_ptr<SocketTransport>& pSocket, std::string apdu,
                  std::vector<uint8_t>& response) {
...
int openConnection(std::shared_ptr<SocketTransport>& pSocket) {
    if (!pSocket->isConnected()) {
        if (!pSocket->openConnection()) return FAILURE;
    } else {
...
int processInputFile() {
    ...
    std::shared_ptr<SocketTransport> pSocket = getInstance();
...
int main(int argc, char* argv[]) {
    ...
    if (provisionStatusSet) getProvisionStatus();
    if (lockProvisionSet) lockProvision();
    return SUCCESS;
}

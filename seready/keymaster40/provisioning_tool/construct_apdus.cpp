...
static int getBootParameterBlobValue(Json::Value& bootParamsObj, const char* key,
                                     std::vector<uint8_t>& blob);

// Print usage.
void usage() {
    printf("Usage: Please give jason files with values as input to generate the apdus command. "
           "Please refer to sample_json files available in the folder for reference. Sample json "
           "files are written using hardcode parameters to be used for testing setup on "
           "cuttlefilsh emulator and goldfish emulators\n");
    printf("construct_apdus [options]\n");
    printf("Valid options are:\n");
    printf("-h, --help    show this help message and exit.\n");
    printf("-v, --km_version version \t Version of the keymaster (40 or 41 for respective "
           "keymaster version; 100 for keymint) \n");
...
int getBootParameterIntValue(Json::Value& bootParamsObj, const char* key, uint32_t* value) {
    ...
    if (val.empty()) return FAILURE;

    if (!val.isInt()) return FAILURE;
...
int getBootParameterBlobValue(Json::Value& bootParamsObj, const char* key,
                              std::vector<uint8_t>& blob) {
    Json::Value val = bootParamsObj[key];
    if (val.empty()) return FAILURE;

    if (!val.isString()) return FAILURE;
...
int processInputFile() {
    ...
    if (keymasterVersion == KEYMASTER_VERSION40 || keymasterVersion == KEYMASTER_VERSION41) {
        ...
        if (0 != processAttestationKey() || 0 != processAttestationCertificateChain() ||
            0 != processAttestationCertificateParams()) {
            return FAILURE;
        }
    } else {
        printf("\n Selected keymint version(%f) for provisioning \n", keymasterVersion);
        if (0 != processDeviceUniqueKey() || 0 != processAdditionalCertificateChain()) {
            return FAILURE;
        }
    }
    if (0 != processAttestationIds() || 0 != processSharedSecret() ||
...
int processAttestationKey() {
    ...
    if (!keyFile.isNull()) {
        ...
        keys.add(privateKey);
        keys.add(publicKey);
        std::vector<uint8_t> keyPairData = keys.encode();
        ...
        input.add(std::move(keyPairData));
        ...
        if (certChainFiles.isArray()) {
            for (uint32_t i = 0; i < certChainFiles.size(); i++) {
                if (certChainFiles[i].isString()) {
                    ...
                    if (SUCCESS !=
                        readDataFromFile(certChainFiles[i].asString().data(), certData)) {
                        printf("\n Failed to read the Root certificate\n");
                        return FAILURE;
                    }
                } else {
                    printf("\n Fail: Only proper certificate paths as a string is allowed inside "
                           "the json file. \n");
                    return FAILURE;
                }
            }
...
int processAttestationCertificateParams() {
    ...
    if (!certChainFile.isNull()) {
        ...
        if (certChainFile.isArray()) {
            ...
            /* Subject, AuthorityKeyIdentifier and Expirty time of the root certificate are required
             * by javacard. */
    ...
    if (!attestIds.isNull()) {
        ...
        for (std::string key : keys) {
            ...
            if (0 == key.compare("brand")) {
                ...
                map.add(kTagAttestationIdManufacturer,
                        std::vector<uint8_t>(idVal.begin(), idVal.end()));
...
int processSetBootParameters() {
    ...
    if (!bootParamsObj.isNull()) {
        ...
        if (SUCCESS !=
            getBootParameterIntValue(bootParamsObj, "boot_patch_level", &bootPatchLevel)) {
            printf("\n Invalid value for boot_patch_level or boot_patch_level tag missing\n");
            return FAILURE;
        }
        if (SUCCESS !=
            getBootParameterBlobValue(bootParamsObj, "verified_boot_key", verifiedBootKey)) {
            printf("\n Invalid value for verified_boot_key or verified_boot_key tag missing\n");
            return FAILURE;
        }
        if (SUCCESS != getBootParameterBlobValue(bootParamsObj, "verified_boot_key_hash",
                                                 verifiedBootKeyHash)) {
            printf("\n Invalid value for verified_boot_key_hash or verified_boot_key_hash tag "
                   "missing\n");
            return FAILURE;
        }
...
int ecRawKeyFromPKCS8(const std::vector<uint8_t>& pkcs8Blob, std::vector<uint8_t>& secret,
    ...
    if (!evpkey) {
        printf("\n Failed to decode private key from PKCS8, Error: %u", ERR_peek_last_error());
        return FAILURE;
    }
    ...
    if (!ec_key.get()) {
        printf("\n Failed to create EC_KEY, Error: %u", ERR_peek_last_error());
        return FAILURE;
    }
    ...
    if (USHRT_MAX >= inputData.size()) {
        ...
        if (inputData.size() > 0) {
            inputData.insert(inputData.begin(),
                             static_cast<uint8_t>(inputData.size() & 0xFF));  // LSB
            inputData.insert(inputData.begin(),
                             static_cast<uint8_t>(inputData.size() >> 8));  // MSB
        }
...
int main(int argc, char* argv[]) {
    int c;
    struct option longOpts[] = {{"km_version", required_argument, NULL, 'v'},
    ...
    if (keymasterVersion == -1 || inputFileName.empty() || outputFileName.empty() ||
        optind < argc) {
    ...
    if (keymasterVersion != KEYMASTER_VERSION40 && keymasterVersion != KEYMASTER_VERSION41 &&
        keymasterVersion != KEYMINT_VERSION) {

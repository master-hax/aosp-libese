...
public class KMRepository implements KMUpgradable {
  ...
  private static final short OPER_DATA_LEN =
      OPERATION_HANDLE_ENTRY_SIZE + KMOperationState.MAX_DATA;
  ...
  public KMRepository(boolean isUpgrading) {
    ...
    operationStateTable[0] = JCSystem
        .makeTransientByteArray(DATA_ARRAY_LENGTH, JCSystem.CLEAR_ON_RESET);
  ...
  /**
   * This function sets the power reset status flag to its default value.
   */
  ...
  public KMOperationState findOperation(byte[] buf, short off, short len) {
    ...
    while (index < MAX_OPS) {
      ...
      if (0 == Util
          .arrayCompare(buf, off, oprTableData, (short) (offset + OPERATION_HANDLE_OFFSET), len)) {
        return KMOperationState
            .read(oprTableData, (short) (offset + OPERATION_HANDLE_OFFSET), oprTableData,
  ...
  public void persistOperation(byte[] data, short opHandle, KMOperation op) {
    ...
    while (index < MAX_OPS) {
      ...
      if ((1 == oprTableData[(short) (offset + OPERATION_HANDLE_STATUS_OFFSET)])
          ...
          KMByteBlob.cast(buf).getStartOff(),
          KMByteBlob.cast(buf).length()))) {
        Util.arrayCopy(data, (short) 0, oprTableData,
            (short) (offset + OPERATION_HANDLE_ENTRY_SIZE),

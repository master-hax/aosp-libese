...
public class KMDecoder {
  ...
  public KMDecoder() {
    ...
    scratchBuf = (short[]) JCSystem
        .makeTransientShortArray(SCRATCH_BUF_SIZE, JCSystem.CLEAR_ON_RESET);
    bufferRef[0] = null;
  ...
  private short decodeEnumArrayTag(short exp) {
    ...
    return KMEnumArrayTag
        .instance(scratchBuf[TAG_KEY_OFFSET], decode(KMEnumArrayTag.cast(exp).getValues()));
  }
  ...
  private short decodeByteBlob(short exp) {
    ...
    short inst = KMByteBlob
        .instance((byte[]) bufferRef[0], scratchBuf[START_OFFSET], payloadLength);

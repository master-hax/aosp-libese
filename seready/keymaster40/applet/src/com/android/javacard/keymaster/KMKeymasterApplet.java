...
public class KMKeymasterApplet extends Applet implements AppletEvent, ExtendedLength {
  ...
  private static final byte INIT_STATE = KM_BEGIN_STATE + 1;
  private static final byte IN_PROVISION_STATE = KM_BEGIN_STATE + 2;
  private static final byte ACTIVE_STATE = KM_BEGIN_STATE + 3;
    ...
    try {
      ...
      switch (apduIns) {
        case INS_SET_BOOT_PARAMS_CMD:
          ...
          // active state. If host does not support boot signal event, then allow this instruction
          // any time.
  ...
  private void processDeviceLockedCmd(APDU apdu) {
    ...
    tmpVariables[0] = decoder
        .decode(tmpVariables[0], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET],
    ...
    if (KMInteger.compare(verTime, lastDeviceLockedTime) > 0) {
      // The Timestamp in the verification token is UINT64.
      Util.arrayFillNonAtomic(scratchPad, (short) 0, (short) KMInteger.UINT_64, (byte) 0);
  ...
  private void processGetHwInfoCmd(APDU apdu) {
    ...
    bufferProp[BUF_LEN_OFFSET] = encoder
        .encode(respPtr, (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET]);
    // send buffer to host
    sendOutgoing(apdu);
  }
  ...
  private void processGetProvisionStatusCmd(APDU apdu) {
    ...
    bufferProp[BUF_START_OFFSET] = repository.allocAvailableMemory();
    bufferProp[BUF_LEN_OFFSET] = encoder
        .encode(tmpVariables[0], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET]);
    sendOutgoing(apdu);
  }
  ...
  private void processGetHmacSharingParamCmd(APDU apdu) {
    ...
    // Encode the response
    bufferProp[BUF_LEN_OFFSET] = encoder
        .encode(tmpVariables[3], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET]);
    sendOutgoing(apdu);
  }
  ...
  private void processComputeSharedHmacCmd(APDU apdu) {
    ...
    // Decode the arguments
    tmpVariables[0] = decoder
        .decode(tmpVariables[2], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET],
            bufferProp[BUF_LEN_OFFSET]);
    //reclaim memory
    repository.reclaimMemory(bufferProp[BUF_LEN_OFFSET]);
  ...
  private void processUpgradeKeyCmd(APDU apdu) {
    ...
    // Decode the arguments
    tmpVariables[2] = decoder
        .decode(tmpVariables[1], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET],
            bufferProp[BUF_LEN_OFFSET]);
    //reclaim memory
    repository.reclaimMemory(bufferProp[BUF_LEN_OFFSET]);
    ...
    isKeyUpgradeRequired |= isKeyUpgradeRequired(KMType.VENDOR_PATCH_LEVEL,
        repository.getVendorPatchLevel());
    isKeyUpgradeRequired |= isKeyUpgradeRequired(KMType.BOOT_PATCH_LEVEL,
        repository.getBootPatchLevel());
  ...
  private void processAttestKeyCmd(APDU apdu) {
    ...
    bufferProp[BUF_START_OFFSET] =
        encoder.encodeCert((byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET], cert.getCertStart(),
            cert.getCertLength(),
            buildErrorStatus(KMError.OK));
    bufferProp[BUF_LEN_OFFSET] = (short) (cert.getCertLength() + (cert.getCertStart()
        - bufferProp[BUF_START_OFFSET]));
    sendOutgoing(apdu);
  }
  ...
  private void processFinishOperationCmd(APDU apdu) {
    ...
    // Encode the response
    bufferProp[BUF_LEN_OFFSET] = encoder
        .encode(tmpVariables[2], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET]);
    sendOutgoing(apdu);
  }
  ...
  private void processBeginOperationCmd(APDU apdu) {
    ...
    KMArray.cast(tmpVariables[1]).add((short) 3, tmpVariables[3]);
    // Decode the arguments
    args = decoder.decode(tmpVariables[1], (byte[]) bufferRef[0], bufferProp[BUF_START_OFFSET],
        bufferProp[BUF_LEN_OFFSET]);
    //reclaim memory
    repository.reclaimMemory(bufferProp[BUF_LEN_OFFSET]);

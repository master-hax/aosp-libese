...
enum class Operation { Update = 0, Finish = 1 };

inline ErrorCode hidlParamSet2OperatinInfo(const hidl_vec<KeyParameter>& params,
                                           OperationInfo& info) {
...
ErrorCode OperationContext::setOperationInfo(uint64_t operationHandle, KeyPurpose purpose,
                                             Algorithm alg, const hidl_vec<KeyParameter>& params) {
...
ErrorCode OperationContext::validateInputData(uint64_t operHandle, Operation opr,
                                              const std::vector<uint8_t>& actualInput,
    ...
    if (opr == Operation::Finish) {
        // If it is observed in finish operation that buffered data + input data exceeds the
        // MAX_ALLOWED_INPUT_SIZE then combine both the data in a single buffer. This helps in
        // making sure that no data is left out in the buffer after finish opertion.
...
ErrorCode OperationContext::update(uint64_t operHandle, const std::vector<uint8_t>& actualInput,
    ...
    if (ErrorCode::OK !=
        (errorCode = validateInputData(operHandle, Operation::Update, actualInput, input))) {
        return errorCode;
    }
    ...
    if (input.size() > MAX_ALLOWED_INPUT_SIZE) {
        ...
        if (extraData > 0) {
            ...
            if (ErrorCode::OK !=
                (errorCode = handleInternalUpdate(operHandle, finalInput.data(), finalInput.size(),
                                                  Operation::Update, cb))) {
                return errorCode;
            }
        }
    } else {
        if (ErrorCode::OK != (errorCode = handleInternalUpdate(
                                  operHandle, input.data(), input.size(), Operation::Update, cb))) {
            return errorCode;
        }
    }
    return errorCode;
...
ErrorCode OperationContext::finish(uint64_t operHandle, const std::vector<uint8_t>& actualInput,
                                   sendDataToSE_cb cb) {
    ErrorCode errorCode = ErrorCode::OK;
    std::vector<uint8_t> input;
    /* Validate the input data */
    if (ErrorCode::OK !=
        (errorCode = validateInputData(operHandle, Operation::Finish, actualInput, input))) {
        return errorCode;
    }
    ...
    if (input.size() > MAX_ALLOWED_INPUT_SIZE) {
        ...
        if (ErrorCode::OK !=
            (errorCode = handleInternalUpdate(operHandle, input.data(), input.size(),
                                              Operation::Finish, cb, true))) {
            return errorCode;
        }
    }
...
/*
 * This function is called for only Symmetric operations. It calculates the length of the data to be
 * sent to the Applet by considering data from both of the sources i.e. buffered data and input
 * data. Only block aligned length of data is sent to the Applet i.e. multiples of 16 for AES or
 * multiples of 8 for DES/TDES. It first Copies the data to the out buffer from buffered data and
 * then the remaining from the input data. If the buffered data is empty then it copies data to out
 * buffer from only input and similarly if the input is empty then it copies from only buffer.
 * Incase if only a portion of the input data is consumed then the remaining portion of input data
 * is buffered. For AES/TDES Decryption operations with PKCS7 padding and for AES GCM operations a
 * block size of data is always buffered. This is done to make sure that there will be always a
 * block size of data left for finish operation so that the Applet may remove the PKCS7 padding if
 * any or get the tag data for AES GCM operation for authentication purpose. Once the data from the
 * buffer is consumed then the buffer is cleared.
 */
ErrorCode OperationContext::getBlockAlignedData(uint64_t operHandle, uint8_t* input,
                                                size_t input_len, Operation opr,
    ...
    if (opr == Operation::Finish) {
        ...
        // For symmetric ciphers, decryption operation and PKCS7 padding mode or AES GCM operation
        // save the last 16 bytes of block and send this block in finish operation. This is done to
        // make sure that there will be always a 16 bytes of data left for finish operation so that
        // javacard Applet may remove PKCS7 padding if any or get the tag data for AES GCM operation
        ...
        // Copy data to be send to SE from buffer, only if atleast a minimum block aligned size is
        // available.
    ...
    if (dataToSELen > 0) {
        // If buffer length is greater than the data length to be send to SE, then input data
        // consumed is 0. That means all the data to be send to SE is consumed from the buffer. The
        // buffer length might become greater than dataToSELen in the cases where we are saving the
        // last block of data i.e. AES/TDES Decryption with PKC7Padding or AES GCM Decryption
        // operations.
...
ErrorCode OperationContext::handleInternalUpdate(uint64_t operHandle, uint8_t* data, size_t len,
                                                 Operation opr, sendDataToSE_cb cb, bool finish) {
    ErrorCode errorCode = ErrorCode::OK;
    std::vector<uint8_t> out;
    if (Algorithm::AES == operationTable[operHandle].info.alg ||
        Algorithm::TRIPLE_DES == operationTable[operHandle].info.alg) {
        /*Symmetric */
        if (ErrorCode::OK != (errorCode = getBlockAlignedData(operHandle, data, len, opr, out))) {
            return errorCode;
        }
        ...
        // 3. if the operation is GCM Mode. Even though there is no data to be send there could be
        // AAD data to be sent to javacard.
        ...
        if (operationTable[operHandle].info.purpose == KeyPurpose::DECRYPT ||
            ...
            // In case of Decrypt operation or Sign operation with no digest case, buffer the data
            // in update call and send it to SE in finish call.
            if (finish) {
                ...
                if (operationTable[operHandle].data.buf_len != 0) {
                    memset(operationTable[operHandle].data.buf, 0x00,
                           sizeof(operationTable[operHandle].data.buf));
                ...
                // For strongbox keymaster, in NoDigest case the length of the input message for RSA
                // should not be more than 256 and for EC it should not be more than 32. This
                // validation is already happening in validateInputData function.
            ...
            // if len=0, then no need to call the callback, since there is no information to be send
            // to javacard,

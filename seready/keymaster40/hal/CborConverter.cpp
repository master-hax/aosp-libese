...
bool CborConverter::addKeyparameters(Array& array,
                                     const android::hardware::hidl_vec<KeyParameter>& keyParams) {
    Map map;
    ...
    for (size_t i = 0; i < keyParams.size(); i++) {
        ...
        case TagType::ENUM_REP:
            enum_repetition[static_cast<uint64_t>(param.tag)].push_back(
                static_cast<uint8_t>(param.f.integer));
            break;
        case TagType::ULONG:
...
bool CborConverter::getHmacSharingParameters(const std::unique_ptr<Item>& item, const uint32_t pos,
    ...
    // Seed
    if (!getBinaryArray(arrayItem, 0, params.seed)) return ret;

    // nonce
    if (!getBinaryArray(arrayItem, 1, paramValue)) return ret;
    memcpy(params.nonce.data(), paramValue.data(), paramValue.size());
    ret = true;
    return ret;
}

bool CborConverter::addVerificationToken(Array& array, const VerificationToken& verificationToken,
...
bool CborConverter::getHardwareAuthToken(const std::unique_ptr<Item>& item, const uint32_t pos,
                                         HardwareAuthToken& token) {
    bool ret = false;
    // challenge
    if (!getUint64<uint64_t>(item, pos, token.challenge)) return ret;
    // userId
    if (!getUint64<uint64_t>(item, pos + 1, token.userId)) return ret;
    // AuthenticatorId
    if (!getUint64<uint64_t>(item, pos + 2, token.authenticatorId)) return ret;
    // AuthType
    uint64_t authType;
    if (!getUint64<uint64_t>(item, pos + 3, authType)) return ret;
    token.authenticatorType = static_cast<HardwareAuthenticatorType>(authType);
    // Timestamp
    if (!getUint64<uint64_t>(item, pos + 4, token.timestamp)) return ret;
    // MAC
    if (!getBinaryArray(item, pos + 5, token.mac)) return ret;
    ret = true;
    return ret;
}

bool CborConverter::getVerificationToken(const std::unique_ptr<Item>& item, const uint32_t pos,
    ...
    // timestamp
    if (!getUint64<uint64_t>(item, pos + 1, token.timestamp)) return ret;

    // List of KeyParameters
    if (!getKeyParameters(item, pos + 2, token.parametersVerified)) return ret;

    // AuthenticatorId
    uint64_t val;
    if (!getUint64<uint64_t>(item, pos + 3, val)) return ret;
    token.securityLevel = static_cast<SecurityLevel>(val);

    // MAC
    if (!getBinaryArray(item, pos + 4, token.mac)) return ret;
    ret = true;
    return ret;
}

bool CborConverter::getKeyParameters(const std::unique_ptr<Item>& item, const uint32_t pos,
                                     android::hardware::hidl_vec<KeyParameter>& keyParams) {
    bool ret = false;

...
#define JAVACARD_KEYMASTER_NAME "JavacardKeymaster4.0Device v1.0"
...
static inline std::unique_ptr<se_transport::TransportFactory>& getTransportFactoryInstance() {
    ...
    if (pTransportFactory == nullptr) {
        ...
        pTransportFactory = std::unique_ptr<se_transport::TransportFactory>(
            new se_transport::TransportFactory(isEmulator));
...
template <typename T = ErrorCode> static T translateExtendedErrorsToHalErrors(T& errorCode) {
    T err;
...
static ErrorCode generateOperationHandle(uint64_t& oprHandle) {
    ...
    do {
        keymaster_error_t err =
            GenerateRandom(reinterpret_cast<uint8_t*>(&oprHandle), (size_t)sizeof(oprHandle));
        if (err != KM_ERROR_OK) {
...
static ErrorCode createOprHandleEntry(uint64_t origOprHandle, uint64_t keymasterSrc,
...
static ErrorCode getOrigOperationHandle(uint64_t halGeneratedOperationHandle,
                                        uint64_t& origOprHandle) {
    std::map<uint64_t, std::pair<uint64_t, uint64_t>>::iterator it =
        operationTable.find(halGeneratedOperationHandle);
 ...
 */
static uint32_t handleErrorCode(const std::unique_ptr<OperationContext>& oprCtx,
                                uint32_t errorCode) {
...
static std::tuple<std::unique_ptr<Item>, T>
decodeData(CborConverter& cb, const std::vector<uint8_t>& response, bool hasErrorCode,
           const std::unique_ptr<OperationContext>& oprCtx) {
...
ErrorCode encodeParametersVerified(const VerificationToken& verificationToken,
...
ErrorCode prepareCborArrayFromKeyData(const hidl_vec<KeyParameter>& keyParams, KeyFormat keyFormat,
                                      const hidl_vec<uint8_t>& blob, cppbor::Array& array) {
    ErrorCode errorCode = ErrorCode::OK;
    AuthorizationSet paramSet;
    ...
    if (keyFormat == KeyFormat::PKCS8) {
        ...
        if (KM_ALGORITHM_RSA == algorithm) {
            ...
            if (ErrorCode::OK !=
                (errorCode = rsaRawKeyFromPKCS8(std::vector<uint8_t>(blob), privExp, modulus))) {
                return errorCode;
            }
            array.add(privExp);
            array.add(modulus);
            ...
            EcCurve curve;
            if (ErrorCode::OK != (errorCode = ecRawKeyFromPKCS8(std::vector<uint8_t>(blob), privKey,
                                                                pubKey, curve))) {
                return errorCode;
            }
            array.add(privKey);
...
ErrorCode parseWrappedKey(const hidl_vec<uint8_t>& wrappedKeyData, std::vector<uint8_t>& iv,
                          std::vector<uint8_t>& transitKey, std::vector<uint8_t>& secureKey,
                          std::vector<uint8_t>& tag, hidl_vec<KeyParameter>& authList,
                          KeyFormat& keyFormat, std::vector<uint8_t>& wrappedKeyDescription) {
    KeymasterBlob kmIv;
...
ErrorCode constructApduMessage(Instruction& ins, std::vector<uint8_t>& inputData,
...
ErrorCode sendData(Instruction ins, std::vector<uint8_t>& inData, std::vector<uint8_t>& response) {
    ...
    if (ret != ErrorCode::OK) {
        LOG(ERROR) << "error in constructApduMessage cmd: " << (int32_t)ins
                   << " status: " << (int32_t)ret;
        return ret;
    }

    if (!getTransportFactoryInstance()->sendData(apdu.data(), apdu.size(), response)) {
        LOG(ERROR) << "error in sendData cmd: " << (int32_t)ins
                   << " status: " << (int32_t)ErrorCode::SECURE_HW_COMMUNICATION_FAILED;
...
static ErrorCode setAndroidSystemProperties(CborConverter& cborConverter_,
    ...
    array.add(GetOsVersion()).add(GetOsPatchlevel()).add(GetVendorPatchlevel());
    ...
    if (ErrorCode::OK != errorCode)
        LOG(ERROR) << "Failed to set os_version, os_patchlevel and vendor_patchlevel err: "
                   << (int32_t)errorCode;
...
JavacardKeymaster4Device::JavacardKeymaster4Device()
    : softKm_(new ::keymaster::AndroidKeymaster(
          ...
          kOperationTableSize,
          keymaster::MessageVersion(keymaster::KmVersion::KEYMASTER_4, 0 /* km_date */))),
      oprCtx_(new OperationContext()), isEachSystemPropertySet(false) {
    ...
    if (ErrorCode::OK == setAndroidSystemProperties(cborConverter_, oprCtx_)) {
        LOG(ERROR)
            << "javacard strongbox : setAndroidSystemProperties from constructor - successful";
...
Return<void> JavacardKeymaster4Device::getHardwareInfo(getHardwareInfo_cb _hidl_cb) {
    ...
    if (ret == ErrorCode::OK) {
        // Skip last 2 bytes in cborData, it contains status.
        std::tie(item, ret) = decodeData(
            cborConverter_, std::vector<uint8_t>(resp.begin(), resp.end() - 2), false, oprCtx_);
        if (item != nullptr) {
            ...
            if (!cborConverter_.getUint64(item, 0, securityLevel) ||
                ...
                _hidl_cb(static_cast<SecurityLevel>(securityLevel), jcKeymasterName,
                         jcKeymasterAuthor);
                return Void();
            }
        }
        ...
        // It should not come here, but incase if for any reason SB keymaster fails to
        // getHardwareInfo return proper values from HAL.
        LOG(ERROR) << "Failed to fetch getHardwareInfo from javacard returning fixed values from "
                      "HAL itself";
...
JavacardKeymaster4Device::getHmacSharingParameters(getHmacSharingParameters_cb _hidl_cb) {
    ...
    if (ErrorCode::OK == errorCode) {
        ...
        std::tie(item, errorCode) =
            decodeData(cborConverter_, std::vector<uint8_t>(cborData.begin(), cborData.end() - 2),
                       true, oprCtx_);
        if (item != nullptr) {
            if (!cborConverter_.getHmacSharingParameters(item, 1, hmacSharingParameters)) {
                LOG(ERROR) << "javacard strongbox : Failed to convert cbor data of "
                              "INS_GET_HMAC_SHARING_PARAM_CMD";
                errorCode = ErrorCode::UNKNOWN_ERROR;
            }
        }
        LOG(ERROR)
            << "javacard strongbox : received getHmacSharingParameter from Javacard - successful";
    }
...
JavacardKeymaster4Device::computeSharedHmac(const hidl_vec<HmacSharingParameters>& params,
    ...
    if (!isEachSystemPropertySet) {
        ...
        LOG(ERROR) << "javacard strongbox : setAndroidSystemProperties from ComputeSharedHmac - "
                      "successful ";
    ...
    if (ErrorCode::OK == errorCode) {
        ...
        if (item != nullptr) {
            ...
            if (!cborConverter_.getBinaryArray(item, 1, bstr)) {
                LOG(ERROR)
                    << "INS_COMPUTE_SHARED_HMAC_CMD: failed to convert cbor sharing check value";
                errorCode = ErrorCode::UNKNOWN_ERROR;
            } else {
                sharingCheck = bstr;
            }
    ...
    if (errorCode == ErrorCode::OK) {
        ...
        if (item != nullptr) {
            if (!cborConverter_.getBinaryArray(item, 1, keyBlob) ||
                ...
                errorCode = ErrorCode::UNKNOWN_ERROR;
                LOG(ERROR) << "INS_GENERATE_KEY_CMD: error while converting cbor data: "
                           << (int32_t)errorCode;
            }
        }
    }
...
Return<void> JavacardKeymaster4Device::importKey(const hidl_vec<KeyParameter>& keyParams,
    ...
    if (ErrorCode::OK !=
        (errorCode = prepareCborArrayFromKeyData(keyParams, keyFormat, keyData, subArray))) {
        LOG(ERROR) << "INS_IMPORT_KEY_CMD Error in while creating cbor data from key data:"
                   << (int32_t)errorCode;
        _hidl_cb(errorCode, keyBlob, keyCharacteristics);
        return Void();
    }
    ...
    if (errorCode == ErrorCode::OK) {
        ...
        if (item != nullptr) {
            if (!cborConverter_.getBinaryArray(item, 1, keyBlob) ||
                ...
                keyCharacteristics.hardwareEnforced.setToExternal(nullptr, 0);
                errorCode = ErrorCode::UNKNOWN_ERROR;
                LOG(ERROR) << "INS_IMPORT_KEY_CMD: error while converting cbor data, status: "
                           << (int32_t)errorCode;
            }
        }
    }
...
Return<void> JavacardKeymaster4Device::importWrappedKey(
    ...
    if (ErrorCode::OK !=
        (errorCode = parseWrappedKey(wrappedKeyData, iv, transitKey, secureKey, tag, authList,
    ...
    if (errorCode == ErrorCode::OK) {
        ...
        if (item != nullptr) {
            if (!cborConverter_.getBinaryArray(item, 1, keyBlob) ||
                ...
                errorCode = ErrorCode::UNKNOWN_ERROR;
                LOG(ERROR)
                    << "INS_IMPORT_WRAPPED_KEY_CMD: error while converting cbor data, status: "
                    << (int32_t)errorCode;
            }
        }
    }
    ...
    if (errorCode == ErrorCode::OK) {
        ...
        if (item != nullptr) {
            if (!cborConverter_.getKeyCharacteristics(item, 1, keyCharacteristics)) {
                ...
                errorCode = ErrorCode::UNKNOWN_ERROR;
                LOG(ERROR)
                    << "INS_GET_KEY_CHARACTERISTICS_CMD: error while converting cbor data, status: "
                    << (int32_t)errorCode;
            }
        }
    }
    ...
    if (errorCode == ErrorCode::OK) {
        ...
        if (item != nullptr) {
            if (!cborConverter_.getMultiBinaryArray(item, 1, temp)) {
                errorCode = ErrorCode::UNKNOWN_ERROR;
                LOG(ERROR) << "INS_ATTEST_KEY_CMD: error in converting cbor data, status: "
                           << (int32_t)errorCode;
            } else {
                cborData.clear();
                cborOutData.clear();
                ...
                if (errorCode == ErrorCode::OK) {
                    ...
                    if (item != nullptr) {
                        ...
                        if (!cborConverter_.getBinaryArray(item, 1, chain)) {
                            errorCode = ErrorCode::UNKNOWN_ERROR;
                            LOG(ERROR) << "attestkey INS_GET_CERT_CHAIN_CMD: errorn in converting "
                                          "cbor data, status: "
                                       << (int32_t)errorCode;
                        } else {
                            if (ErrorCode::OK == (errorCode = getCertificateChain(chain, temp))) {
                                ...
                                LOG(ERROR) << "Error in attestkey getCertificateChain: "
                                           << (int32_t)errorCode;
                            }
                        }
                    }
    ...
    if (errorCode == ErrorCode::OK) {
        ...
        if (item != nullptr) {
            if (!cborConverter_.getBinaryArray(item, 1, upgradedKeyBlob)) {
                errorCode = ErrorCode::UNKNOWN_ERROR;
                LOG(ERROR) << "INS_UPGRADE_KEY_CMD: error in converting cbor data, status: "
                           << (int32_t)errorCode;
            }
        }
    }
...
Return<void> JavacardKeymaster4Device::begin(KeyPurpose purpose, const hidl_vec<uint8_t>& keyBlob,
    ...
    if (KeyPurpose::ENCRYPT == purpose || KeyPurpose::VERIFY == purpose) {
        ...
        /* For Symmetric key operation, the BeginOperation returns KM_ERROR_INCOMPATIBLE_ALGORITHM
         * error. */
        ...
        if (response.error != KM_ERROR_INCOMPATIBLE_ALGORITHM) { /*Incompatible algorithm could be
                                                                    handled by JavaCard*/
            errorCode = legacy_enum_conversion(response.error);
            ...
            if (errorCode == ErrorCode::OK) {
                ...
                if (errorCode != ErrorCode::OK)
                    LOG(ERROR)
                        << "INS_BEGIN_OPERATION_CMD error while creating new operation handle: "
                        << (int32_t)errorCode;
            }
    ...
    // keyCharacteristics.hardwareEnforced is required to store algorithm, digest and padding values
    // in operationInfo structure. To retrieve keyCharacteristics.hardwareEnforced, call
    // getKeyCharacateristics. By calling getKeyCharacateristics also helps in finding a corrupted
    // keyblob.
            ...
            if (errorCode == ErrorCode::OK) {
                ...
                if (item != nullptr) {
                    if (!cborConverter_.getKeyParameters(item, 1, outParams) ||
                        ...
                        operationHandle = 0;
                        LOG(ERROR)
                            << "INS_BEGIN_OPERATION_CMD: error in converting cbor data, status: "
                            << (int32_t)errorCode;
                    } else {
...
Return<void> JavacardKeymaster4Device::update(uint64_t halGeneratedOprHandle,
                                              const hidl_vec<KeyParameter>& inParams,
    ...
    if (ErrorCode::OK !=
        (errorCode = getOrigOperationHandle(halGeneratedOprHandle, operationHandle))) {
...
Return<void> JavacardKeymaster4Device::finish(uint64_t halGeneratedOprHandle,
                                              const hidl_vec<KeyParameter>& inParams,
                                              const hidl_vec<uint8_t>& input,
...
Return<ErrorCode> JavacardKeymaster4Device::abort(uint64_t halGeneratedOprHandle) {
    ...
    if (response.error == KM_ERROR_INVALID_OPERATION_HANDLE) {
        ...
        if (errorCode == ErrorCode::OK) {
            // Skip last 2 bytes in cborData, it contains status.
            std::tie(item, errorCode) = decodeData(
                cborConverter_, std::vector<uint8_t>(cborOutData.begin(), cborOutData.end() - 2),
                true, oprCtx_);
        }
    }

...
class CborConverter {
  public:
    ...
    /**
     * Parses the input data which is in CBOR format and returns a Tuple of Item pointer and the
     * first element in the item pointer.
     */
    template <typename T = ErrorCode>
    std::tuple<std::unique_ptr<Item>, T> decodeData(const std::vector<uint8_t>& response,
                                                    bool hasErrorCode) {
        const uint8_t* pos;
        ...
        if (item != nullptr && hasErrorCode) {
            if (MajorType::ARRAY == getType(item)) {
                if (!getErrorCode(item, 0, errorCode)) item = nullptr;
                ...
                item =
                    nullptr; /*Already read the errorCode. So no need of sending item to client */
            }
        }
     ...
     */
    bool getHmacSharingParameters(const std::unique_ptr<Item>& item, const uint32_t pos,
     ...
     */
    bool addKeyparameters(Array& array, const android::hardware::hidl_vec<KeyParameter>& keyParams);
     ...
     */
    bool addVerificationToken(Array& array, const VerificationToken& verificationToken,
  ...
  private:
     ...
     * unsigned integers.
     */
    bool getKeyParameter(
        const std::pair<const std::unique_ptr<Item>&, const std::unique_ptr<Item>&> pair,
        std::vector<KeyParameter>& keyParam);

    /**
     * Get the sub item pointer from the root item pointer at the given position.
     */
    inline void getItemAtPos(const std::unique_ptr<Item>& item, const uint32_t pos,
                             std::unique_ptr<Item>& subItem) {
        Array* arr = nullptr;
...
template <typename T> bool CborConverter::getUint64(const std::unique_ptr<Item>& item, T& value) {
    bool ret = false;

...
public class KMAttestationCertImpl implements KMAttestationCert {
  ...
  public KMAttestationCert notAfter(
    ...
    if (!derEncoded) {
      ...
      notAfter =
          KMByteBlob.instance(
              KMByteBlob.cast(usageExpiryTimeObj).getBuffer(),
  ...
  private static void pushTbsCert(boolean rsaCert, boolean rsa) {
    ...
    // subject
    pushBytes(
        KMByteBlob.cast(subjectName).getBuffer(),
        KMByteBlob.cast(subjectName).getStartOff(),
        KMByteBlob.cast(subjectName).length());
    pushValidity();
    // issuer - der encoded
    pushBytes(
    ...
    // Serial Number
    pushBytes(
        KMByteBlob.cast(serialNum).getBuffer(),
        KMByteBlob.cast(serialNum).getStartOff(),
        KMByteBlob.cast(serialNum).length());
  ...
  public boolean subjectName(short sub) {
    ...
    Util.arrayCopyNonAtomic(KMByteBlob.cast(sub).getBuffer(), KMByteBlob.cast(sub).getStartOff(),

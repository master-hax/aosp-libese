...
class CborConverter {
  public:
    ...
    template <typename T> std::optional<T> getUint64(const unique_ptr<Item>& item) {
        T value;
        if ((item == nullptr) ||
    ...
    inline std::optional<keymaster_error_t> getErrorCode(const std::unique_ptr<Item>& item,
                                                         const uint32_t pos) {

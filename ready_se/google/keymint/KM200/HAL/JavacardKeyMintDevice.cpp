...
// Import wrapped key is divided into 2 stage operation: sendBeginImportWrappedKeyCmd()
// and sendFinishImportWrappedKeyCmd() to reduce the ram consumption in the Applet.
// In sendBeginImportWrappedKeyCmd, Applet does
// 1. Parse wrapping keyblob
// 2. Decrypt the encrypted transport key with wrapping keyblob using RSA-OAEP
// 3. XOR the decrypted transport key with the provided masking key
// 4. Store the AES-GCM key (transport key decypted and X'ored)
// In sendFinishImportWrappedKeyCmd, Applet does
// 1. Decrypt the wrapped key using the stored AES-GCM key.
// 2. import the key and create encrypted keyblob.
...
ScopedAStatus JavacardKeyMintDevice::begin(KeyPurpose purpose, const std::vector<uint8_t>& keyBlob,
    ...
    auto retErr = card_->sendEarlyBootEndedEvent(false /* earlyBootEventTriggered */);
...
JavacardKeyMintDevice::deviceLocked(bool passwordOnly,
    ...
    if (err != KM_ERROR_OK) {
        LOG(ERROR) << "Error in sending deviceLocked request.";
        return km_utils::kmError2ScopedAStatus(err);
    }
    return ScopedAStatus::ok();
}
...
ScopedAStatus JavacardKeyMintDevice::earlyBootEnded() {
    // if send earlyBootEnded event fails, JavacardSecureElement sets the
    // isEarlyBootEventPending flag to true. Later, when JavacardSharedSecret
    // calls JavacardSecureElement to send computeSharedSecret to SE,
    // the JavacardSecureElement tries to send earlyBootEnded event
    // if the isEarlyBootEventPending flag is true. Once earlyBootEnded event
    // is successfully sent it sets the isEarlyBootEventPending to flase.
    // The isEarlyBootEventPending is maintained inside JavacardSecureElement
    // because it is the common place from where all the commands are sent to SE.
    auto err = card_->sendEarlyBootEndedEvent(true /* earlyBootEventTriggered */);
    if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardKeyMintDevice::getKeyCharacteristics(
    ...
    if (!optKeyChars) {
        LOG(ERROR) << "Error in parsing the KeyCharacteristics.";
        return km_utils::kmError2ScopedAStatus(KM_ERROR_UNKNOWN_ERROR);
    }
...
ScopedAStatus JavacardKeyMintDevice::getRootOfTrustChallenge(array<uint8_t, 16>* challenge) {
    ...
    if (!optChallenge) {
        LOG(ERROR) << "Error in parsing cbor response for getRootOfTrustChallenge.";
        return km_utils::kmError2ScopedAStatus(KM_ERROR_UNKNOWN_ERROR);
    }

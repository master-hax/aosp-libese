...
public class KMDecoder {
  ...
  private short decodeEnumAndCreateInstance(short instanceType) {
    byte[] buffer = (byte[]) bufferRef[0];
    if ((buffer[scratchBuf[START_OFFSET]] & MAJOR_TYPE_MASK) != UINT_TYPE) {
      ISOException.throwIt(ISO7816.SW_DATA_INVALID);
    }
    short addInfo = (short) (buffer[scratchBuf[START_OFFSET]] & ADDITIONAL_MASK);
    ...
    byte origValue = buffer[scratchBuf[START_OFFSET]];
    if (addInfo < UINT8_LENGTH) {
      // In this case additional info is the actual enum value.
      buffer[scratchBuf[START_OFFSET]] = (byte) addInfo;
    }
    short len = (short) ((addInfo > UINT8_LENGTH) ? ((addInfo == UINT32_LENGTH) ? 4 : 2) : 1);
    short ptr = createInstanceEnumType(instanceType, buffer, scratchBuf[START_OFFSET], len);
    buffer[scratchBuf[START_OFFSET]] = origValue;

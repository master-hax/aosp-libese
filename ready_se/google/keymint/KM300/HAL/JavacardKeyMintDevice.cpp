...
ScopedAStatus JavacardKeyMintDevice::getHardwareInfo(
  ...
  info->securityLevel =
      static_cast<SecurityLevel>(std::move(optSecLevel.value()));
  info->versionNumber = static_cast<int32_t>(std::move(optVersion.value()));
  return ScopedAStatus::ok();
}

ScopedAStatus JavacardKeyMintDevice::generateKey(
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_GENERATE_KEY_CMD, array);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardKeyMintDevice::addRngEntropy(
    const vector<uint8_t>& data) {
  cppbor::Array request;
  // add key data
  request.add(Bstr(data));
  auto [item, err] =
      card_->sendRequest(Instruction::INS_ADD_RNG_ENTROPY_CMD, request);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardKeyMintDevice::importKey(
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_IMPORT_KEY_CMD, request);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardKeyMintDevice::importWrappedKey(
    ...
    const vector<uint8_t>& wrappingKeyBlob, const vector<uint8_t>& maskingKey,
    const vector<KeyParameter>& unwrappingParams, int64_t passwordSid,
    int64_t biometricSid, KeyCreationResult* creationResult) {
  cppbor::Array request;
  ...
  keymaster_error_t errorCode =
      parseWrappedKey(wrappedKeyData, iv, transitKey, secureKey, tag, authList,
  ...
  // begin import
  std::tie(item, errorCode) = sendBeginImportWrappedKeyCmd(
      transitKey, wrappingKeyBlob, maskingKey, unwrappingParams);
  if (errorCode != KM_ERROR_OK) {
  ...
  std::tie(item, errorCode) = sendFinishImportWrappedKeyCmd(
      authList, keyFormat, secureKey, tag, iv, wrappedKeyDescription,
      passwordSid, biometricSid);
  if (errorCode != KM_ERROR_OK) {
    LOG(ERROR)
        << "Error in send finish import wrapped key in importWrappedKey.";
    return km_utils::kmError2ScopedAStatus(errorCode);
  }
  auto optKeyBlob = cbor_.getByteArrayVec(item, 1);
  auto optKeyChars = cbor_.getKeyCharacteristics(item, 2);
...
JavacardKeyMintDevice::sendBeginImportWrappedKeyCmd(
    const std::vector<uint8_t>& transitKey,
  ...
  return card_->sendRequest(Instruction::INS_BEGIN_IMPORT_WRAPPED_KEY_CMD,
                            request);
}
...
JavacardKeyMintDevice::sendFinishImportWrappedKeyCmd(
    ...
    const std::vector<uint8_t>& wrappedKeyDescription, int64_t passwordSid,
  ...
  return card_->sendRequest(Instruction::INS_FINISH_IMPORT_WRAPPED_KEY_CMD,
                            request);
}

ScopedAStatus JavacardKeyMintDevice::upgradeKey(
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_UPGRADE_KEY_CMD, request);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardKeyMintDevice::deleteKey(const vector<uint8_t>& keyBlob) {
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_DELETE_KEY_CMD, request);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardKeyMintDevice::begin(
    KeyPurpose purpose, const std::vector<uint8_t>& keyBlob,
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_BEGIN_OPERATION_CMD, array);
  if (err != KM_ERROR_OK) {
  ...
  result->operation = ndk::SharedRefBase::make<JavacardKeyMintOperation>(
      ...
      static_cast<BufferingMode>(optBufMode.value()), optMacLength.value(),
      card_);
  return ScopedAStatus::ok();
}

ScopedAStatus JavacardKeyMintDevice::deviceLocked(
    bool passwordOnly, const std::optional<TimeStampToken>& timestampToken) {
  Array request;
  int8_t password = 1;
  if (!passwordOnly) {
    password = 0;
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_DEVICE_LOCKED_CMD, request);
  if (err != KM_ERROR_OK) {
    return km_utils::kmError2ScopedAStatus(err);
  }
  return ScopedAStatus::ok();
...
ScopedAStatus JavacardKeyMintDevice::getKeyCharacteristics(
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_GET_KEY_CHARACTERISTICS_CMD, request);
  if (err != KM_ERROR_OK) {
...
keymaster_error_t JavacardKeyMintDevice::parseWrappedKey(
    const vector<uint8_t>& wrappedKeyData, std::vector<uint8_t>& iv,
    std::vector<uint8_t>& transitKey, std::vector<uint8_t>& secureKey,
    std::vector<uint8_t>& tag, vector<KeyParameter>& authList,
  ...
  keymaster_error_t error = parse_wrapped_key(
      KeymasterKeyBlob(keyMaterial), &kmIv, &kmTransitKey, &kmSecureKey, &kmTag,
...
ScopedAStatus JavacardKeyMintDevice::getRootOfTrustChallenge(
  ...
  std::move(optChallenge->begin(), optChallenge->begin() + 16,
            challenge->begin());
  return ScopedAStatus::ok();
}

ScopedAStatus JavacardKeyMintDevice::getRootOfTrust(
    const array<uint8_t, 16>& /*challenge*/, vector<uint8_t>* /*rootOfTrust*/) {
  return km_utils::kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}

ScopedAStatus JavacardKeyMintDevice::sendRootOfTrust(
    const vector<uint8_t>& rootOfTrust) {
  cppbor::Array request;
  request.add(EncodedItem(rootOfTrust));  // taggedItem.
  auto [item, err] =
      card_->sendRequest(Instruction::INS_SEND_ROT_DATA_CMD, request);
  if (err != KM_ERROR_OK) {

...
keymaster_error_t JavacardSecureElement::sendEarlyBootEndedEvent(
    bool eventTriggered) {
  ...
  if (err != KM_ERROR_OK) {
    // Incase of failure cache the event and send in the next immediate request
    // to Applet.
...
keymaster_error_t JavacardSecureElement::constructApduMessage(
    Instruction& ins, std::vector<uint8_t>& inputData,
...
keymaster_error_t JavacardSecureElement::sendData(
    Instruction ins, std::vector<uint8_t>& inData,
  ...
  if ((response.size() <= 2) ||
      (getApduStatus(response) != APDU_RESP_STATUS_OK)) {

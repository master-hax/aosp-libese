...
KeyParameter kmEnumParam2Aidl(const keymaster_key_param_t& param) {
  switch (param.tag) {
    case KM_TAG_PURPOSE:
      return KeyParameter{
          Tag::PURPOSE, KeyParameterValue::make<KeyParameterValue::keyPurpose>(
    ...
    case KM_TAG_PADDING:
      return KeyParameter{
          Tag::PADDING, KeyParameterValue::make<KeyParameterValue::paddingMode>(
...
KeyParameter kmParam2Aidl(const keymaster_key_param_t& param) {
  ...
  switch (typeFromTag(param.tag)) {
    ...
    case KM_ULONG_REP:
      return KeyParameter{
          tag, KeyParameterValue::make<KeyParameterValue::longInteger>(
                   param.long_integer)};
      break;

    case KM_DATE:
      return KeyParameter{tag,
                          KeyParameterValue::make<KeyParameterValue::dateTime>(
                              param.date_time)};
      break;
    ...
    case KM_BYTES:
      return {tag,
              KeyParameterValue::make<KeyParameterValue::blob>(std::vector(
                  param.blob.data, param.blob.data + param.blob.data_length))};
      break;
...
keymaster_key_param_t aidlEnumVal2Km(keymaster_tag_t km_tag,
                                     const KeyParameterValue& value) {
  return value.getTag() == aidl_tag
             ? keymaster_param_enum(
                   km_tag, static_cast<uint32_t>(value.get<aidl_tag>()))
             : kInvalidTag;
}
...
keymaster_key_param_t aidlEnumParam2Km(const KeyParameter& param) {
  ...
  switch (tag) {
    ...
    case KM_TAG_USER_AUTH_TYPE:
      return aidlEnumVal2Km<KeyParameterValue::hardwareAuthenticatorType>(
          tag, param.value);
    case KM_TAG_ORIGIN:
...
keymaster_error_t legacyHardwareAuthToken(
    const HardwareAuthToken& aidlToken, LegacyHardwareAuthToken* legacyToken) {
      ...
      case KM_UINT_REP:
        set.params[i] =
            param.value.getTag() == KeyParameterValue::integer
                ? keymaster_param_int(
                      tag, param.value.get<KeyParameterValue::integer>())
                : kInvalidTag;
        break;
      ...
      case KM_ULONG_REP:
        set.params[i] =
            param.value.getTag() == KeyParameterValue::longInteger
                ? keymaster_param_long(
                      tag, param.value.get<KeyParameterValue::longInteger>())
                : kInvalidTag;
        break;
      ...
      case KM_DATE:
        set.params[i] =
            param.value.getTag() == KeyParameterValue::dateTime
                ? keymaster_param_date(
                      tag, param.value.get<KeyParameterValue::dateTime>())
                : kInvalidTag;
        break;

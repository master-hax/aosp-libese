...
class JavacardSecureElement {
  public:
    explicit JavacardSecureElement(shared_ptr<ITransport> transport, uint32_t osVersion,
        ...
        : transport_(transport), osVersion_(osVersion), osPatchLevel_(osPatchLevel),
          ...
          isDeleteAllKeysEventPending(false) {
    ...
    keymaster_error_t sendEarlyBootEndedIfPending(bool eventTriggered);
    keymaster_error_t sendDeleteAllKeysIfPending(bool eventTriggered);
  ...
  private:
    keymaster_error_t sendEvent(Instruction ins, bool* eventPending, bool eventTriggered);
    ...
    bool isDeleteAllKeysEventPending;
    CborConverter cbor_;
};
}  // namespace keymint::javacard

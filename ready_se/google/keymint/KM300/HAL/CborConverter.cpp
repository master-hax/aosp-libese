...
std::optional<uint32_t> aidlEnumParam2Uint32(const KeyParameter& param) {
  ...
  switch (tag) {
    ...
    case KM_TAG_USER_AUTH_TYPE:
      return aidlEnumVal2Uint32<KeyParameterValue::hardwareAuthenticatorType>(
          param.value);
    case KM_TAG_ORIGIN:
...
bool CborConverter::addAttestationKey(
    Array& array, const std::optional<AttestationKey>& attestationKey) {
...
bool CborConverter::addKeyparameters(Array& array,
  ...
  for (auto& param : keyParams) {
    ...
    switch (typeFromTag(tag)) {
      ...
      case KM_ULONG_REP:
        if (param.value.getTag() == KeyParameterValue::longInteger) {
          auto longVal = param.value.get<KeyParameterValue::longInteger>();
          uint_repetition[static_cast<uint64_t>(tag & 0x00000000ffffffff)].add(
              longVal);
        }
        break;
      case KM_DATE:
...
// Array of three maps
std::optional<vector<KeyCharacteristics>> CborConverter::getKeyCharacteristics(
    const unique_ptr<Item>& item, const uint32_t pos) {
  ...
  if (!sbEnf.authorizations.empty())
    keyCharacteristics.push_back(std::move(sbEnf));
  if (!teeEnf.authorizations.empty())
    keyCharacteristics.push_back(std::move(teeEnf));
  if (!swEnf.authorizations.empty())
    keyCharacteristics.push_back(std::move(swEnf));
  return keyCharacteristics;
}

std::optional<std::vector<KeyParameter>> CborConverter::getKeyParameter(
    const std::pair<const std::unique_ptr<Item>&, const std::unique_ptr<Item>&>
        pair) {
...
// array of a blobs
std::optional<vector<Certificate>> CborConverter::getCertificateChain(
    const std::unique_ptr<Item>& item, const uint32_t pos) {
...
std::optional<string> CborConverter::getTextStr(const unique_ptr<Item>& item,
                                                const uint32_t pos) {
...
std::optional<string> CborConverter::getByteArrayStr(
    const unique_ptr<Item>& item, const uint32_t pos) {
...
std::optional<std::vector<uint8_t>> CborConverter::getByteArrayVec(
    const unique_ptr<Item>& item, const uint32_t pos) {
...
std::optional<SharedSecretParameters> CborConverter::getSharedSecretParameters(
    const unique_ptr<Item>& item, const uint32_t pos) {
...
bool CborConverter::addSharedSecretParameters(
    Array& array, const vector<SharedSecretParameters>& params) {
  Array cborParamsVec;
...
bool CborConverter::addTimeStampToken(Array& array,
...
bool CborConverter::addHardwareAuthToken(Array& array,
...
std::optional<TimeStampToken> CborConverter::getTimeStampToken(
    const unique_ptr<Item>& item, const uint32_t pos) {
  ...
  token.timestamp.milliSeconds =
      static_cast<long>(std::move(optTimestampMillis.value()));
  return token;
}

std::optional<Array> CborConverter::getArrayItem(
    const std::unique_ptr<Item>& item, const uint32_t pos) {
  Array array;
  auto arrayItem = getItemAtPos(item, pos);
  if (!arrayItem || (MajorType::ARRAY != getType(arrayItem.value()))) {
...
std::optional<vector<KeyParameter>> CborConverter::getKeyParameters(
    const unique_ptr<Item>& item, const uint32_t pos) {
...
std::tuple<std::unique_ptr<Item>, keymaster_error_t> CborConverter::decodeData(
...
std::optional<keymaster_error_t> CborConverter::getErrorCode(
    const std::unique_ptr<cppbor::Item>& item, const uint32_t pos) {

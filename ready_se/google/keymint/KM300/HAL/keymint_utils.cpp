...
constexpr char kPlatformVersionRegex[] =
...
uint32_t getOsVersion(const char* version_str) {
  ...
  int not_match = regexec(&regex, version_str, kPlatformVersionMatchCount,
                          matches, 0 /* flags */);
  regfree(&regex);
  if (not_match) {
    return 0;
  ...
  uint32_t subminor =
      match_to_uint32(version_str, matches[kSubminorVersionMatch]);
...
uint32_t getPatchlevel(const char* patchlevel_str, PatchlevelOutput detail) {
  ...
  int not_match = regexec(&regex, patchlevel_str, kPatchlevelMatchCount,
                          matches, 0 /* flags */);
  regfree(&regex);
  if (not_match) {
    return 0;

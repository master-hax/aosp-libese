...
keymaster_error_t OmapiTransport::initialize() {
  ...
  if (omapiSeService == nullptr) {
    ...
    return static_cast<keymaster_error_t>(
        ErrorCode::HARDWARE_NOT_YET_AVAILABLE);
  }
...
keymaster_error_t OmapiTransport::sendData(const vector<uint8_t>& inData,
                                           vector<uint8_t>& output) {
  if (!isConnected()) {
    // Try to initialize connection to eSE
    LOG(INFO)
        << "Failed to send data, try to initialize connection SE connection";
    auto res = initialize();
    if (res != KM_ERROR_OK) {

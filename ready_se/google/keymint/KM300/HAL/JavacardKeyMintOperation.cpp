...
ScopedAStatus JavacardKeyMintOperation::updateAad(
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_UPDATE_AAD_OPERATION_CMD, request);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardKeyMintOperation::update(
    const vector<uint8_t>& input, const optional<HardwareAuthToken>& authToken,
    const optional<TimeStampToken>& timestampToken, vector<uint8_t>* output) {
  ...
  DataView view = {
      .buffer = {}, .data = input, .start = 0, .length = input.size()};
...
ScopedAStatus JavacardKeyMintOperation::finish(
  ...
  const vector<uint8_t> confToken =
      confirmationToken.value_or(vector<uint8_t>());
  const vector<uint8_t> inData = input.value_or(vector<uint8_t>());
  DataView view = {
      .buffer = {}, .data = inData, .start = 0, .length = inData.size()};
...
ScopedAStatus JavacardKeyMintOperation::abort() {
  ...
  request.add(Uint(opHandle_));
  auto [item, err] =
      card_->sendRequest(Instruction::INS_ABORT_OPERATION_CMD, request);
  opHandle_ = 0;
  buffer_.clear();
  return km_utils::kmError2ScopedAStatus(err);
}
...
void JavacardKeyMintOperation::blockAlign(DataView& view, uint16_t blockSize) {
  ...
  if (view.buffer.empty() && view.data.empty()) {
    ...
    buffer_.insert(buffer_.end(), view.buffer.begin() + offset,
                   view.buffer.end());
  } else {
    if (offset < view.buffer.size()) {
      ...
      buffer_.insert(buffer_.end(), view.data.begin() + offset,
                     view.data.end());
    }
  }
...
uint16_t JavacardKeyMintOperation::getDataViewOffset(DataView& view,
                                                     uint16_t blockSize) {
  uint16_t offset = 0;
...
keymaster_error_t JavacardKeyMintOperation::updateInChunks(
    DataView& view, HardwareAuthToken& authToken,
...
vector<uint8_t> JavacardKeyMintOperation::popNextChunk(DataView& view,
                                                       uint32_t chunkSize) {
...
keymaster_error_t JavacardKeyMintOperation::sendUpdate(
    const vector<uint8_t>& input, const HardwareAuthToken& authToken,
  ...
  auto [item, error] =
      card_->sendRequest(Instruction::INS_UPDATE_OPERATION_CMD, request);
...
keymaster_error_t JavacardKeyMintOperation::sendFinish(
    const vector<uint8_t>& data, const vector<uint8_t>& sign,
    const HardwareAuthToken& authToken, const TimeStampToken& timestampToken,
  ...
  auto [item, err] =
      card_->sendRequest(Instruction::INS_FINISH_OPERATION_CMD, request);
  if (err != KM_ERROR_OK) {
    return err;
  }
  auto optTemp = cbor_.getByteArrayVec(item, 1);

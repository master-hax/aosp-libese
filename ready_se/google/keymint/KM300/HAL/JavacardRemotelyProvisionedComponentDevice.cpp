...
keymaster_error_t translateRkpErrorCode(keymaster_error_t error) {
  switch (static_cast<int32_t>(-error)) {
    ...
    case kStatusInvalidMac:
      return static_cast<keymaster_error_t>(
          BnRemotelyProvisionedComponent::STATUS_INVALID_MAC);
...
ScopedAStatus JavacardRemotelyProvisionedComponentDevice::getHardwareInfo(
  ...
  info->versionNumber =
      static_cast<int32_t>(std::move(optVersionNumber.value()));
  info->supportedEekCurve =
      static_cast<int32_t>(std::move(optSupportedEekCurve.value()));
  info->uniqueId = std::move(optUniqueId.value());
  info->supportedNumKeysInCsr =
      static_cast<int32_t>(std::move(optMinSupportedKeysInCsr.value()));
  return ScopedAStatus::ok();
}

ScopedAStatus
JavacardRemotelyProvisionedComponentDevice::generateEcdsaP256KeyPair(
    bool testMode, MacedPublicKey* macedPublicKey,
    std::vector<uint8_t>* privateKeyHandle) {
  cppbor::Array array;
  array.add(testMode);
  auto [item, err] =
      card_->sendRequest(Instruction::INS_GENERATE_RKP_KEY_CMD, array);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardRemotelyProvisionedComponentDevice::beginSendData(
  ...
  array.add(challenge);
  auto [item, err] =
      card_->sendRequest(Instruction::INS_BEGIN_SEND_DATA_CMD, array);
  if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardRemotelyProvisionedComponentDevice::updateMacedKey(
  ...
  for (auto& macedPublicKey : keysToSign) {
    ...
    auto [item, err] =
        card_->sendRequest(Instruction::INS_UPDATE_KEY_CMD, array);
    if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardRemotelyProvisionedComponentDevice::finishSendData(
  ...
  if (!optCEncryptProtectedHeader || !optSignature || !optVersion ||
      !optRespFlag) {
...
ScopedAStatus JavacardRemotelyProvisionedComponentDevice::getDiceCertChain(
    std::vector<uint8_t>& diceCertChain) {
  uint32_t respFlag = 0;
  do {
    auto [item, err] =
        card_->sendRequest(Instruction::INS_GET_DICE_CERT_CHAIN_CMD);
    if (err != KM_ERROR_OK) {
...
ScopedAStatus JavacardRemotelyProvisionedComponentDevice::getUdsCertsChain(
  ...
  do {
    ...
    respFlag = optRespFlag.value();
    udsCertsChain.insert(udsCertsChain.end(), optUdsCertData->begin(),
                         optUdsCertData->end());
  } while (respFlag != 0);
  return ScopedAStatus::ok();
}
...
ScopedAStatus
JavacardRemotelyProvisionedComponentDevice::generateCertificateRequest(
    bool, const std::vector<MacedPublicKey>&, const std::vector<uint8_t>&,
    const std::vector<uint8_t>&, DeviceInfo*, ProtectedData*,
    std::vector<uint8_t>*) {
  return km_utils::kmError2ScopedAStatus(
      static_cast<keymaster_error_t>(STATUS_REMOVED));
}
...
JavacardRemotelyProvisionedComponentDevice::generateCertificateRequestV2(
  ...
  auto ret = beginSendData(keysToSign, challenge, &deviceInfo,
                           &csrPayloadSchemaVersion, &certificateType);
  ...
  ret =
      finishSendData(coseEncryptProtectedHeader, signature, version, respFlag);
  if (!ret.isOk()) return ret;

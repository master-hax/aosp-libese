...
public class CBORDecoder extends CBORBase {
     ...
     */
    public byte getMajorType() { return (byte)((getRawByte() >>> 5) & MAJOR_TYPE_MASK); }
     ...
     * of the additional information field (increases offset by the number of length bytes). Throws
     * an ISOExeption if the major type is not correct.
     *
    ...
    public short readEncodedInteger(byte[] output, short offset) {
        ...
        if (size == 1) {  // Check for special case (integer could be encoded in first type)
            ...
            Util.arrayCopyNonAtomic(getBuffer(), getCurrentOffsetAndIncrease((short)(1 + size)),
                                    output, offset, size);
        }
    ...
    public short readRawByteArray(byte[] outBuffer, short outOffset, short length) {
        ...
        length = (short)(Util.arrayCopyNonAtomic(getBuffer(), getCurrentOffset(), outBuffer,
                                                 outOffset, length) -
                         outOffset);
     ...
     */
    private byte readRawByte() { return getBuffer()[mStatusWords[0]++]; }
}

...
public class Context {
    ...
    public Context(byte[] buffer, MdlSpecifications mdlSpecs) {
        ...
        for (byte i = 0; i < MAX_DOC_REQUESTS; i++) {
            ...
            // type and itemsRequests bytes used for reader auth. The pointer will have two fields
            // i.e. length and start in the mBuffer Note we are currently only supported doc type
            // processing and reader auth processing. In the future we may support the individual
            // element processing.
            ...
            // two pointers - one pointer is index in doc table and another pointer is the index in
            // MSO table.
        }
    ...
    void setChunkSize(short chunkSize) { mChunkSize[0] = chunkSize; }

...
/**
 * This class handles the DocumentRequest processing. For each document request it enumerates
 * the data by selecting it from PresentationPkg. Thus, Document Request is like a select
 * command whereas PresentationPkg is like a table. Each document Request stores the start and
 * end of element values for all the data items requested in the document request. It also
 * generates device signature for the response.
 */
public class DocumentRequest {
    ...
    /**
     * Mac0 structure and generates the Mac Tag. Mac0 Structure is partially defined MAC0_STRUCT -
     * to which detached content is added as payload. Detached content is as follows:
     *
    ...
    private static final byte[] MAC0_STRUCT = {
        ...
        0x43,        //         'C'
        0x30,        //         '0'
        0x43,        //     bstr .size 3 (contents are encoded CBOR protected map)
        (byte)0xA1,  //         map of 1 entry
        0x01,        //             1
        0x05,        //             5
        0x40,        //     Empty byte string (external AAD)
                     //     payload bstr goes here.
    };

    private static final byte[] SIGN1_Signature1 = {
        // tstr containing "Signature1"
    ...
    /**
     * Generates the signature structure and then verifies the signature.  The detached content
     * is as follows:
     *
     ...
     *     ]
     */
    public boolean performReaderAuth(SEProvider se, byte[] buf, short itemsBytesStart,
                                     short itemsBytesLen, byte[] sessionTrans,
                                     short sessionTransStart, short sessionTransLen, short alg,
                                     byte[] pub, short pubKeyStart, short pubKeyLen, byte[] sign,
                                     short signStart, short signLen, byte[] scratch,
                                     short scratchStart, short scratchLen) {

        // Convert the signature to ASN1 format
        short len = se.convertCoseSign1SignatureToAsn1(sign, signStart, signLen, scratch,
                                                       scratchStart, (short)(scratchLen - signLen));
        if (len < 0) {
            return false;
        }
        signLen = len;
        ...
        // Get the verifier
        Signature verifier =
            se.getVerifier(pub, pubKeyStart, pubKeyLen, alg, Signature.MODE_VERIFY);
        if (verifier == null) {
            return false;
        }
        ...
        short readerAuthLen =
            (short)(1                                                 // array header
                    + mMdlSpecifications.readerAuthentication.length  // "ReaderAuthentication"
                    + sessionTransLen + itemsBytesLen);

        // Calculate the cbor tagged byte string length
        ...
        mEncoder.startByteString((short)0);    // external aad
        mEncoder.startByteString(payloadLen);  // payload
        ...
        mEncoder.startArray((short)3);  // ReaderAuthentication
        ...
        // Digest encoded data, Session Transcript and item bytes.
        ...
        boolean ret =
            verifier.verify(buf, itemsBytesStart, itemsBytesLen, sign, signStart, signLen);
        return ret;
    }
    ...
    public byte[] getDeviceAuthMacTag() { return mMacTag; }

    public MdocPresentationPkg getDocument() { return (MdocPresentationPkg)mPresentationPkg[0]; }

    public void setDocument(MdocPresentationPkg doc) { mPresentationPkg[0] = doc; }

    public void setError() { error[0] = true; }

    public boolean isError() { return error[0]; }

    public void setErrorCode(short err) { errorCode[0] = err; }
    ...
    boolean init(MdocPresentationPkg doc, Session session, byte[] buf, short nameSpacesStart,
                 short nameSpacesLen, byte[] scratch, short scratchStart, short scratchLen) {
        reset();
        ...
        short numNamespaces = mDecoder.readMajorType(CBORBase.TYPE_MAP);

        for (short i = 0; i < numNamespaces; i++) {
            ...
            for (short j = 0; j < dataElemCount; j++) {
                ...
                mDecoder.skipEntry();  // Ignore intentToRetain
            }
        ...
        // Generate device authentication MAC.
        try {
    ...
    public short decodeCoseSign1(byte[] buf, short readerAuth, short readerAuthLen,
                                 short[] tmpArray, byte[] scratch, short scratchStart) {
        short signLen = -1;
    ...
    public boolean generateDeviceAuthMacTag(Session session, byte[] scratchPad, short start,
                                            short len) {
        ...
        short sessionTransStart = getSessionTranscriptStart(session.mSessionTranscriptBytes,
        ...
        short detachedContentLen = (short)(
            ...
            sessionTransLength +
            // doc type length
            docTypeLength + (short)4);  // no namespaces thus only 3 bytes of header
    ...
    void initDeviceAuth(Session session, byte[] scratch, short start, short len) {
        ...
        short secretLen =
            mKeyAgreement.generateSecret(scratch, start, pubLen, scratch, secretStart);
        short outStart =
            (short)(secretStart + secretLen);  // Util.arrayCopyNonAtomic(Session.mSalt, (short) 0,
                                               // scratch, saltStart,saltLen);
    ...
    short getNsCount() { return (short)(mNsTableEnd / NS_ARR_ENTRY_SIZE); }

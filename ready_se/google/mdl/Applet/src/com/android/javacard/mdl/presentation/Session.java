...
public class Session {
    ...
    public Session(SEProvider se, MdlSpecifications mdlSpecs) {
        ...
        mReaderEncodedKeyBytes = JCSystem.makeTransientByteArray(READER_ENC_KEY_BYTES_BUF_SIZE,
                                                                 JCSystem.CLEAR_ON_DESELECT);
        mDevicePrivateKey = (ECPrivateKey)KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE,
                                                              KeyBuilder.LENGTH_EC_FP_256, false);

        // TODO for JCOP we can replace the following with transient memory using alternative
        // buildKey method.
        mDeviceKey = (AESKey)KeyBuilder.buildKey(KeyBuilder.TYPE_AES_TRANSIENT_DESELECT,
                                                 KeyBuilder.LENGTH_AES_256, false);
        mReaderKey = (AESKey)KeyBuilder.buildKey(KeyBuilder.TYPE_AES_TRANSIENT_DESELECT,
                                                 KeyBuilder.LENGTH_AES_256, false);
        mHmacKey =
            (HMACKey)KeyBuilder.buildKey(KeyBuilder.TYPE_HMAC_TRANSIENT_RESET, (short)512, false);
        mEReaderKeyPub = (ECPublicKey)KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PUBLIC,
                                                          KeyBuilder.LENGTH_EC_FP_256, false);
    ...
    private void readerKey(byte[] scratchPad, short scratchOffset, short scratchLen) {
        ...
        //    (short) 0));
        short offset = coseKey(mReaderEncodedKeyBytes, (short)2,
    ...
    Signature computeSessionDataAndSecrets() {
        ...
        try {
            ...
            saltLen = messageDigestSha256(mSessionTranscriptBytes, (short)2,
                                          Util.getShort(mSessionTranscriptBytes, (short)0), mSalt,
                                          (short)0);
            if (saltLen != SALT_LEN) {
            ...
            hkdf(scratchPad, secretStart, secretLen, mMdlSpecifications.deviceSecretInfo,
                 scratchPad, outStart);
            mDeviceKey.setKey(scratchPad, outStart);
            hkdf(scratchPad, secretStart, secretLen, mMdlSpecifications.readerSecretInfo,
                 scratchPad, outStart);
    ...
    public short hkdf(byte[] secret, short secretStart, short secretLen, byte[] info, byte[] out,
                      short outStart) {
        return hkdf(mSalt, (short)0, (short)mSalt.length, secret, secretStart, secretLen, info, out,
                    outStart);
    }

    public short hkdf(byte[] salt, short saltStart, short saltLen, byte[] secret, short secretStart,
                      short secretLen, byte[] info, byte[] out, short outStart) {
    ...
    public short messageDigestSha256(byte[] input, short inputStart, short inputLen, byte[] out,
                                     short outStart) {
    ...
    private void encodeSessionTranscript(byte[] sessionTranscript, byte[] handoverSelectMsg,
        ...
        totalLen +=
            (short)4;  // cbor overhead of DeviceEngagementBytes - will always be > 23 and < 256
        ...
        offset = Util.arrayCopyNonAtomic(handoverSelectMsg, handoverSelectMsgStart,
                                         sessionTranscript, offset, handoverSelectMsgLen);
    ...
    boolean isSessionInitialized() { return Util.getShort(mSessionTranscriptBytes, (short)0) > 0; }

    short encryptDecryptData(byte[] mBuffer, short dataStart, short dataLen, AESKey key,
                             short counter, boolean encrypt, short[] retVal, boolean justUpdate) {
        ...
        if (encrypt) {
            return mSEProvider.aesGCMEncryptOneShot(key, mBuffer, dataStart, dataLen, mBuffer,
                                                    dataStart, scratch, scratchStart, nonceLen,
                                                    null, (short)0, (short)0, justUpdate);
        } else {
    ...
    void beginIncrementalEncryption(short counter, byte[] scratch, short scratchStart,
        ...
        mSEProvider.beginAesGcmOperation(mDeviceKey, true, scratch, scratchStart,
                                         SEProvider.AES_GCM_NONCE_LENGTH, null, (short)0, (short)0);
    }
     ...
     */
    short encryptDataIncrementally(byte[] buf, short start, short len, byte[] scratch,
        ...
        dataEnd =
            mSEProvider.encryptDecryptInPlace(buf, start, len, scratch, scratchStart, scratchLen);
    ...
    short finishIncrementalEncryption(byte[] buf, short start, short len) {
        short index = mSEProvider.doAesGcmOperation(buf, start, len, buf, start, false);
        return (short)(index - start);
    }

    private short generateNonce(boolean encrypt, short counter, byte[] scratch,
                                short scratchStart) {
    ...
    public short coseKey(byte[] buffer, short start, short len, short keyId, short[] structure) {
        ...
        if (buffer[structure[0]] != MdlSpecifications.COSE_KEY_KTY_VAL_EC2 ||
            ...
            structure[3] != 1 || structure[4] == 0 || structure[5] != 34 || structure[6] == 0 ||
            structure[7] != 34) {
            ISOException.throwIt(ISO7816.SW_DATA_INVALID);
        }
        return offset;

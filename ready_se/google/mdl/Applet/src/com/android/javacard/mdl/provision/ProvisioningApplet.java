...
public class ProvisioningApplet extends Applet implements ExtendedLength {
     ...
     * 1:CBOR uint maxSlots, 2:CBOR uint maxDocSize, 3:CBOR uint minExtendedApduBufSize, 4: CBOR
     * uint number of pre-allocated slots, - each slot equal to maxDocSize. }
     */
    ...
    private ProvisioningApplet(SEProvider se) {
        ...
        mScratch = JCSystem.makeTransientByteArray(mSEProvider.MAX_SCRATCH_SIZE,
                                                   JCSystem.CLEAR_ON_DESELECT);
        heap = JCSystem.makeTransientByteArray(mSEProvider.MAX_BUFFER_SIZE,
                                               JCSystem.CLEAR_ON_DESELECT);
    ...
    private static void createDocument(MDoc doc, boolean testCred, byte[] scratch, short start,
                                       short len) {
        // Reserve the available slot
        doc.reserve();
        // Create the document.
        doc.create(MAX_MDOC_SIZE, scratch, start, len);
        if (testCred) {
    ...
    private static MDoc findDocument(byte slot) { return mDocuments[slot]; }
    ...
    public void receiveIncoming(APDU apdu) {
        ...
        // TODO add logic to handle the extended length buffer. In this case the memory can be
        // reused
    ...
    private boolean handleMdocCommands(short cmd, APDU apdu, byte[] buf, short start, short len) {
        ...
        case CMD_MDOC_CREATE: {
            ...
            handleCreateMdocCred(apdu, slot, testCredential == 1, osVersionStart, osVersionLen,
                                 osPatchLevelStart, osPatchLevelLen, challengeStart, challengeLen,
                                 notBeforeStart, notBeforeLen, notAfterStart, notAfterLen,
                                 creationDateTimeStart, creationDateTimeLen, attAppIdStart,
                                 attAppIdLen, payLoadLength);
            break;
        }
        ...
        case CMD_MDOC_CREATE_PRESENTATION_PKG: {
            ...
            handleCreateAuthKeys(apdu, slot, notBeforeStart, notBeforeLen, notAfterStart,
                                 notAfterLen, testDevAuthPublicKeyStart, testDevAuthPublicKeyLen,
                                 testDevAuthPrivateKeyStart, testDevAuthPrivateKeyLen,
                                 payLoadLength);
            break;
        }
        ...
        case CMD_MDOC_PROVISION_DATA: {
            ...
            handleProvisionData(apdu, slot, op, mDecoder.getCurrentOffset(),
                                len);  // Encrypted data.
        } break;
    ...
    private void handleProvisionData(APDU apdu, byte slot, byte op, short start, short len) {
        ...
        if (op == BEGIN) {
            ...
            mSEProvider.beginAesGcmOperation(doc.getStorageKey(), false, buf, start,
                                             SEProvider.AES_GCM_NONCE_LENGTH, mScratch, (short)0,
                                             credKeyLen);

            // decrypt data - the decrypted data will be in scratch buffer. Scratch will be
            // sufficiently large to hold slightly greater than 64 bytes of the data.
            short end = mSEProvider.encryptDecryptInPlace(buf, encDataStart, encDataLen, mScratch,
                                                          (short)0, (short)mScratch.length);
            ...
            // Now, current offset in decoded stream is pointing to credential data start point, so
            // don't skip one byte. i.e. don't perform mDecoder.increaseOffset((short) 1);
            ...
            mSEProvider.beginAesGcmOperation(doc.getStorageKey(), true, buf, start,
                                             SEProvider.AES_GCM_NONCE_LENGTH, mScratch, (short)0,
                                             credKeyLen);
            ...
            // not block aligned.
            end = mSEProvider.encryptDecryptInPlace(buf, encDataStart, decryptedDataLen, mScratch,
                                                    (short)0, (short)mScratch.length);
            ...
            // Encrypt
            short end = mSEProvider.encryptDecryptInPlace(buf, start, len, mScratch, (short)0,
    ...
    /**
     * This command can be used for starting, updating and finishing swap in. This is indicated by
     * the op parameter.
     */
    private void handleSwapInMdoc(APDU apdu, byte slot, byte op, short start, short len) {
        ...
        if (op == BEGIN) {
            ...
            // decrypt data - the decrypted data will be directly stored in the doc specific flash
            // memory.
            short end = mSEProvider.encryptDecryptInPlace(buf, encDataStart, encDataLen, mScratch,
                                                          (short)0, (short)mScratch.length);
            short size = (short)(end - encDataStart);
    ...
    /**
     * byte slot,byte testCred, short challenge_start, short challenge_len, short keyParams_start,
    ...
    private void handleCreateMdocCred(APDU apdu, byte slot, boolean testCredential,
                                      ...
                                      short notBeforeStart, short notBeforeLen, short notAfterStart,
                                      short notAfterLen, short creationDateTimeStart,
                                      short creationDateTimeLen, short attAppIdStart,
                                      short attAppIdLen, short payLoadLen) {
        // Get the document
        MDoc doc = findDocument(slot);
        if (doc == null || doc.isReserved()) {
            ISOException.throwIt(ISO7816.SW_RECORD_NOT_FOUND);
        ...
        short certLen = mSEProvider.generateCredKeyCert(
            (ECPrivateKey)mAttestKey.getPrivate(),
            (ECPublicKey)(doc.getCredentialKey().getPublic()), buf, osVersionStart, osVersionLen,
            buf, osPatchLevelStart, osPatchLevelLen, buf, challengeStart, challengeLen, buf,
            notBeforeStart, notBeforeLen, buf, notAfterStart, notAfterLen, buf,
            creationDateTimeStart, creationDateTimeLen, buf, attAppIdStart, attAppIdLen,
            testCredential, buf, start, mSEProvider.MAX_BUFFER_SIZE, mScratch, (short)0,
            (short)mScratch.length);
        sendApdu(apdu, start, certLen);
    }

    private void handleCreateAuthKeys(APDU apdu, byte slot, short notBeforeStart,
                                      short notBeforeLen, short notAfterStart, short notAfterLen,
                                      short testDevAuthPublicKeyStart,
                                      short testDevAuthPublicKeyLen,
                                      short testDevAuthPrivateKeyStart,
                                      short testDevAuthPrivateKeyLen, short payloadLen) {
        MDoc doc = findDocument(slot);
        if (doc == null) {
            ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
        }
        ...
        short certLen = mSEProvider.generateSigningKeyCert(
            (ECPublicKey)(mSigningKey.getPublic()),
            (ECPrivateKey)(doc.getCredentialKey().getPrivate()), buf, notBeforeStart, notBeforeLen,
            buf, notAfterStart, notAfterLen, buf, certStart, SEProvider.SIGNING_CERT_MAX_SIZE,
        ...
        short size = ((ECPublicKey)doc.getCredentialKey().getPublic())
                         .getW(mScratch, SEProvider.AES_GCM_NONCE_LENGTH);

        mSEProvider.beginAesGcmOperation(doc.getStorageKey(), true, mScratch, (short)0,
                                         SEProvider.AES_GCM_NONCE_LENGTH, mScratch,
                                         SEProvider.AES_GCM_NONCE_LENGTH, size);

        // Encode the input data which is an array of 2 elements. We also add nonce in the front
        // before the array.
        ...
        // second element is the credential data which is null at this stage so add cbor null
        // type/value
        ...
        short dataEnd = mSEProvider.encryptDecryptInPlace(buf, dataStart, dataLen, mScratch,
                                                          (short)0, (short)mScratch.length);

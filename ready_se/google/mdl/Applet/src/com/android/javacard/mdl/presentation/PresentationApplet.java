...
public class PresentationApplet extends Applet implements ExtendedLength, MdlService {
    ...
    public static final byte[] AID_MDL_DIRECT_ACCESS_APPLET = {(byte)0xA0, 0x00, 0x00, 0x02,
                                                               0x48,       0x04, 0x00};
    public static final byte INS_ENVELOPE = (byte)0xC3;
    public static final byte INS_GET_RESPONSE = (byte)0xC0;
    public static final byte[] AID_NDEF_TAG_APPLET = {(byte)0xD2, 0x76, 0x00, 0x00,
                                                      (byte)0x85, 0x01, 0x01};
    ...
    static final byte[] HANDOVER_MSG_FIXED_PART = {
        ...
        0x69, 0x73, 0x6F, 0x2E, 0x6F, 0x72, 0x67, 0x3A, 0x31, 0x38, 0x30, 0x31, 0x33, 0x3A, 0x64,
        0x65, 0x76, 0x69, 0x63, 0x65, 0x65, 0x6E, 0x67, 0x61, 0x67, 0x65, 0x6D, 0x65, 0x6E, 0x74,
    ...
    private PresentationApplet(SEProvider se) {
        ...
        mPresentationPkgStore.configure(mSEProvider, mMdlSpecifications, MAX_DOCUMENTS_SLOTS,
                                        (byte)1, (short)0x7FFF);
        reset();
    }
    ...
    boolean processReaderAuth(DocumentRequest req, MdocPresentationPkg doc, short[] tmpArray,
                              byte[] buf, short itemsBytesStart, short itemsBytesLen,
                              short readerAuth, short readerAuthLen, byte[] scratch,
        ...
        short sessionTransStart = req.getSessionTranscriptStart(mSession.mSessionTranscriptBytes,
                                                                (short)2, sessionTransLength);
        ...
        return req.performReaderAuth(mSEProvider, buf, itemsBytesStart,
                                     itemsBytesLen,  // item bytes
                                     mSession.mSessionTranscriptBytes, sessionTransStart,
                                     sessionTransLength,
    ...
    private void handleSessionEst(APDU apdu, byte[] buf, short readerKeyStart, short readeKeyLen,
        ...
        Util.arrayCopyNonAtomic(buf, readerKeyStart, mSession.mReaderEncodedKeyBytes, (short)2,
                                readeKeyLen);
    ...
    private void handleSessionData(APDU apdu, byte[] buf, short statusStart, short statusLen,
    ...
    private short decryptSessionData(byte[] buf, short encDataStart, short encDataLen) {
        // Decrypt the data
        encDataLen =
            mSession.encryptDecryptData(buf, encDataStart, encDataLen, mSession.mReaderKey,
                                        mMsgCounter[READER_MSG_COUNTER], false, mRetVal, false);
    ...
    private void handleDeviceRequestAndSendResponse(APDU apdu, byte[] buf, short dataStart,
                                                    short dataLen) {
    ...
    private void processEnvelope(APDU apdu) {
        ...
        short requestType = mSession.isSessionInitialized()
                                ? MdlSpecifications.IND_SESSION_DATA
                                : MdlSpecifications.IND_SESSION_ESTABLISHMENT;
        short offset =
            mMdlSpecifications.decodeStructure(mMdlSpecifications.getStructure(requestType),
        ...
        if (requestType == MdlSpecifications.IND_SESSION_ESTABLISHMENT) {
            handleSessionEst(apdu, mContext.mBuffer, mStructure[0], mStructure[1], mStructure[2],
                             mStructure[3]);
        } else {  // this is session data
            handleSessionData(apdu, mContext.mBuffer, mStructure[0], mStructure[1], mStructure[2],
                              mStructure[3]);
        }
    }
    ...
    private void sendSuccessResponseWithoutDocs(APDU apdu) {
        ...
        // create the headers in apdu buffer. Note: the response length needs to include GCM Auth
        // tag
        short index =
        ...
        short encLen =
            mSession.encryptDecryptData(buf, index, len, mSession.mDeviceKey,
                                        mMsgCounter[DEVICE_MSG_COUNTER], true, mRetVal, false);
    ...
    private void sendSuccessResponseForSessionTermination(APDU apdu) {
        ...
        // "status" : uint
        mEncoder.encodeRawData(mMdlSpecifications.status, (short)0,
                               (short)mMdlSpecifications.status.length);
    ...
    //
    // 1. create the data structure for DocumentRequest data structure for the doc requests and
    // Items Requests. Device request consists of three layer of structures: Device request->
    // docRequest -> ItemsRequest And Items Request consists of Doc Type, etc. This method just
    // creates an index of all the namespaces and corresponding data elements requested.
    ...
    private short processDeviceRequest(byte[] buf, short deviceRequestStart, short deviceRequestLen,
        ...
        mMdlSpecifications.decodeStructure(str, mStructure, buf, deviceRequestStart,
                                           deviceRequestLen);
        ...
        if (Util.arrayCompare(buf, mStructure[0], mMdlSpecifications.DEVICE_REQ_VERSION, (short)0,
                              mStructure[1]) != 0) {
            ISOException.throwIt(ISO7816.SW_DATA_INVALID);
        }
        ...
        for (short i = 0; i < docReqSize; i++) {
            ...
            current = mMdlSpecifications.decodeStructure(str, mStructure, buf, current,
                                                         (short)(next - current));
            if (current != next) {
                ISOException.throwIt(ISO7816.SW_DATA_INVALID);
            }
    ...
    private boolean receiveBytes(APDU apdu) {
        ...
        Util.arrayCopyNonAtomic(
            buf, buf[ISO7816.OFFSET_LC] != 0 ? ISO7816.OFFSET_CDATA : ISO7816.OFFSET_EXT_CDATA,
     ...
     * signed by the issuer "deviceSigned" : DeviceSigned, ; Returned data elements signed by the
     * mdoc ? "errors" : Errors } DocumentError = { DocType => ErrorCode ; Error codes for
     * unreturned documents } IssuerSigned = { ? "nameSpaces" : IssuerNameSpaces, ; Returned data
     * elements "issuerAuth" : IssuerAuth ; Contains the mobile security object (MSO) ; for issuer
     * data authentication } IssuerNameSpaces = { ; Returned data elements for each namespace +
     * NameSpace
     ...
     * element value } DeviceSigned = { "nameSpaces" : DeviceNameSpacesBytes, ; Returned data
     * elements "deviceAuth" : DeviceAuth ; Contains the device authentication ; for mdoc
     * authentication } DeviceNameSpacesBytes = #6.24(bstr .cbor DeviceNameSpaces) DeviceNameSpaces
     * = { * NameSpace => DeviceSignedItems ; Returned data elements for each namespace }
     * DeviceSignedItems = { + DataElementIdentifier => DataElementValue ; Returned data element
     * identifier and value } DeviceAuth = { ; Either signature or MAC for mdoc authentication
     * "deviceSignature" : DeviceSignature // ; "//" means or "deviceMac" : DeviceMac } Where
     * DeviceMac is COSEMac0 - Array of 4 with protected content {alg=5}, no un protected content
     * and payload followed by mac tag. [ << {1: 5} >>, {}, null, h'
     ...
     * :
     * [+Document], ; Returned documents ? "documentErrors": [+DocumentError]; For unreturned
     * documents, optional error codes "status" : uint ; Status code } This method encodes version
     * and status which is always success.
     */
    private short encodeDeviceResponseStartSuccess(CBOREncoder encoder, byte reqCount) {
        // Map of 3 because we do not return error. Out of three elements two are encoded here and
        // the third i.e. array of documents is encoded in separate method which will be called from
        // encodeResponse method.
        short start = encoder.getCurrentOffset();
        encoder.startMap((short)3);

        // version" : "1.0"
        encoder.encodeRawData(mMdlSpecifications.version, (short)0,
                              (short)mMdlSpecifications.version.length);
        encoder.encodeRawData(mMdlSpecifications.DEVICE_REQ_VERSION, (short)0,
                              (short)mMdlSpecifications.DEVICE_REQ_VERSION.length);
        // "status" : 0 - always ok
        encoder.encodeRawData(mMdlSpecifications.status, (short)0,
                              (short)mMdlSpecifications.status.length);
        // 0 - always ok
        encoder.encodeUInt8((byte)0);
        // "documents" : [+Document]
        encoder.encodeRawData(mMdlSpecifications.documents, (short)0,
                              (short)mMdlSpecifications.documents.length);
    ...
    private short encodeDocumentStart(CBOREncoder encoder, DocumentRequest request) {
        ...
        encoder.startMap((short)3);
        // "docType" : DocType
        encoder.encodeRawData(mMdlSpecifications.docType, (short)0,
                              (short)mMdlSpecifications.docType.length);
     ...
     *
     * <p>DeviceSigned = { "nameSpaces" : DeviceNameSpacesBytes, ; Returned data elements
    ...
    private short encodeDeviceSigned(CBOREncoder encoder, DocumentRequest request) {
        short start = encoder.getCurrentOffset();
        // "deviceSigned" : DeviceSigned
        encoder.encodeRawData(mMdlSpecifications.deviceSigned, (short)0,
                              (short)mMdlSpecifications.deviceSigned.length);
        // DeviceSigned
        encoder.startMap((short)2);
        // nameSpaces" : DeviceNameSpacesBytes - empty structure
        encoder.encodeRawData(mMdlSpecifications.nameSpaces, (short)0,
        ...
        encoder.encodeRawData(mMdlSpecifications.deviceAuth, (short)0,
                              (short)mMdlSpecifications.deviceAuth.length);
        // DeviceAuth
        encoder.startMap((byte)1);
        // "deviceMac" : DeviceMac - DeviceMac is Cose structure
        encoder.encodeRawData(mMdlSpecifications.deviceMac, (short)0,
                              (short)mMdlSpecifications.deviceMac.length);
    ...
    short addEnvAndSessionHeaders(byte[] buf, short index, short len, short respLength) {
        ...
        mEncoder.startMap((short)1);
        // "data" : bstr
        mEncoder.encodeRawData(mMdlSpecifications.session_data, (short)0,
                               (short)mMdlSpecifications.session_data.length);
        ...
        return Util.arrayCopyNonAtomic(mScratchPad, (short)0, buf, index,
                                       mEncoder.getCurrentOffset());
    }
    ...
    private short encodeData(CBOREncoder encoder) {
        ...
        while (mContext.mIncrementalResponseState[Context.CURRENT_STATE] != Context.RESP_IDLE) {
            try {
                ...
                // This exception is thrown by encoder when it cannot encode anymore data as it has
                // run out of space.
    ...
    /**
     * This method returns the response to the reader. It checks whether this is the first message
     * in a chain of responses and if so, it handles that differently by calculating entire message
     * length and appending envelope and session headers. Rest of the messages sent incrementally.
     * It sends message les then or equal to chunk size. However, it processes message data greater
     * the chunk size. This is done so that we can take care of element boundaries.
     *
    ...
    private void processGetResponse(APDU apdu, boolean firstMessage) {
        ...
        if (firstMessage) {
            ...
            // session header requires total device response length. This only returns size of data
            // to be encoded. It does not include the Auth tag which will be added to the encrypted
            // device response data.
            mContext.setOutGoing(calculateResponseLength(), getNextDocIndex(INITIAL_DOC_INDEX));

            // Add the envelope and session data headers and resp length includes the auth tag
            // length which will be concatenated with encrypted response data.
            index = addEnvAndSessionHeaders(
                buf, (short)0, mContext.mChunkSize[0],
            ...
            mSession.beginIncrementalEncryption(mMsgCounter[DEVICE_MSG_COUNTER], mScratchPad,
                                                (short)0, (short)mScratchPad.length);
        }
        // Drive the response state machine, which sends the response to individual document
        // requests received in device request one by one - incrementally i.e. in chunks of size =
        // le.
        short len = 0;
        ...
        while (index < mContext.mChunkSize[0]) {
            // if there is any data remaining in context buffer and if yes then copy that to apdu
            // buffer.
            if (mContext.mBufReadIndex[0] < mContext.mBufWriteIndex[0]) {
                ...
                short remBytes = (short)(mContext.mRemainingBytes[0] + mContext.mBufWriteIndex[0] -
                ...
                // Then encrypt the data which may be upto 16 bytes less if the data is not block
                // aligned.
                ...
                mEncoder.init(mContext.mBuffer, mContext.mBufWriteIndex[0],
                ...
                // Encrypt in place. The returned data can be less than the input data if the data
                // is not blocked aligned.
                mContext.mBufWriteIndex[0] = mSession.encryptDataIncrementally(
                    mContext.mBuffer, (short)0, mContext.mBufWriteIndex[0], mScratchPad, (short)0,
                    (short)mScratchPad.length);
            } else if (mContext.mRemainingBytes[0] == 0) {  // Nothing to encode
                // If there is any buffered data in encryption session then copy that.
                mContext.mBufReadIndex[0] = 0;
                mContext.mBufWriteIndex[0] =
                    mSession.readAndClearBufferedData(mContext.mBuffer, (short)0);
                mContext.mBufWriteIndex[0] = mSession.finishIncrementalEncryption(
    ...
    short encodeIssuerSigned(CBOREncoder encoder, DocumentRequest request) {
        short start = encoder.getCurrentOffset();
        encoder.encodeRawData(mMdlSpecifications.issuerSigned, (short)0,
                              (short)mMdlSpecifications.issuerSigned.length);
        encoder.startMap((short)2);
        encoder.encodeRawData(mMdlSpecifications.issuerAuth, (short)0,
                              (short)mMdlSpecifications.issuerAuth.length);
        return (short)(encoder.getCurrentOffset() - start);
    }
     ...
     * schema specified in Mdl specifications. Every switch case is one state and they do two
     * things: 1) action: Does the action required for the current state - mainly encoding the keys
     * of this node and sometime child nodes. 2) change to next state: Sets transition to next
     * state. The next state can be either root node or child node value.
     */
    private short encodeResponse(CBOREncoder encoder) {
        ...
        case Context.RESP_START:
            ...
            if (mContext.mDocumentsCount[0] > 0) {
                mContext.mIncrementalResponseState[Context.CURRENT_DOC] =
                    getNextDocIndex(INITIAL_DOC_INDEX);
                mContext.mIncrementalResponseState[Context.CURRENT_ELEMENT] = 0;
                mContext.mIncrementalResponseState[Context.CURRENT_NAMESPACE] = 0;
                mContext.mIncrementalResponseState[Context.CURRENT_DATA_PTR_END] = 0;
                mContext.mIncrementalResponseState[Context.CURRENT_DATA_PTR_START] = 0;
        ...
        case Context.RESP_DOCUMENT: {
            ...
            if (docIndex < Context.MAX_DOC_REQUESTS) {
                ...
                mContext.mIncrementalResponseState[Context.CURRENT_STATE] =
                    Context.RESP_DEVICE_SIGNED;
            } else {
    ...
    /**
     * The following method does basic chain validation. 1) Extract tbs, keys, alg, signature from
     * the chain. Also, check whether the last cert in the chain is self signed. 2) Check that alg
     ...
     */
    public boolean validateChain(MdocPresentationPkg doc, byte[] buf, short[] tmpArray,
        ...
        while (i > 2) {
            ...
            if (!mX509CertHandler.decodeCert(buf, certStart, certEnd, mRetVal, scratch,
                                             scratchStart, scratchLen))
                return false;
            ...
            if (count > 1 && attKeyLen > 0 &&
                !mSEProvider.validateEcDsaSign(buf, mRetVal[1],
                                               mRetVal[2],  // tbs
                                               mRetVal[0],  // alg
                                               scratch, mRetVal[3],
                                               mRetVal[4],  // sign
    ...
    private boolean processItemsRequest(byte[] buf, short itemsBytesStart, short itemsBytesLen,
        ...
        if (doc != null) {
            // Now initialise the document request which will parse the name spaces and create
            // internal mapping to stored data. Perform readAuth
            if (processReaderAuth(req, doc, mStructure, buf, itemsBytesStart, itemsBytesLen,
                                  readerAuthStart, readerAuthLen, mScratchPad, (short)0,
                                  (short)mScratchPad.length)) {
                return req.init(doc, mSession, buf, nameSpacesStart, nameSpacesLen, mScratchPad,
                                (short)0, (short)mScratchPad.length);
            }
        }
        return false;
    ...
    public Shareable getShareableInterfaceObject(AID clientAID, byte parameter) {
        ...
        case MdlPresentationPkgStore.SERVICE_ID:
            if (Util.arrayCompare(buf, (short)0, DIRECT_ACCESS_PROVISIONING_APPLET_ID, (short)0,
                                  len) != 0) {
                ISOException.throwIt(ISO7816.SW_WRONG_DATA);
            }
    ...
    public short getHandoverSelectMessage(byte[] buf, short start) {
        if (buf == null || (short)buf.length < (short)(start + HANDOVER_MSG_FIXED_PART.length +
        ...
        Util.setShort(mSession.mHandover, Session.DEVICE_ENGAGEMENT_START,

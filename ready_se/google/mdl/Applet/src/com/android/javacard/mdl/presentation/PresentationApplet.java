...
public class PresentationApplet extends Applet implements ExtendedLength, MdlService {
  ...
  private PresentationApplet(SEProvider se) {
    ...
    mPresentationPkgStore.configure(mSEProvider, mMdlSpecifications, MAX_DOCUMENTS_SLOTS, (byte) 1,
        (short) 0x7FFF);
    reset();
  }
  ...
  public static void install(byte[] bArray, short bOffset, byte bLength) {
    SEProvider se = new SEProvider(bArray, bOffset, bLength);
  ...
  boolean processReaderAuth(DocumentRequest req, MdocPresentationPkg doc, short[] tmpArray,
    ...
    short signLen = req.decodeCoseSign1(buf, readerAuth, readerAuthLen, tmpArray,
    ...
    if (!validateChain(doc, buf, tmpArray,
    ...
    return req.performReaderAuth(mSEProvider, buf,
        itemsBytesStart, itemsBytesLen, // item bytes
        mSession.mSessionTranscriptBytes, sessionTransStart, sessionTransLength,
  ...
  private void handleSessionEst(APDU apdu, byte[] buf,
    ...
    Util.arrayCopyNonAtomic(buf, readerKeyStart, mSession.mReaderEncodedKeyBytes,
        (short) 2, readeKeyLen);
  ...
  private void handleSessionData(APDU apdu, byte[] buf,
  ...
  private short decryptSessionData(byte[] buf,
    ...
    encDataLen = mSession.encryptDecryptData(buf, encDataStart, encDataLen,
        mSession.mReaderKey, mMsgCounter[READER_MSG_COUNTER], false, mRetVal, false);
  ...
  private void processEnvelope(APDU apdu) {
    ...
    SEProvider.print(mContext.mBuffer, bufIndex, bufLen);
    short requestType = mSession.isSessionInitialized() ? MdlSpecifications.IND_SESSION_DATA :
        MdlSpecifications.IND_SESSION_ESTABLISHMENT;
    short offset = mMdlSpecifications.decodeStructure(mMdlSpecifications.getStructure(requestType),
  ...
  private void sendSuccessResponseWithoutDocs(APDU apdu) {
    ...
    short encLen = mSession.encryptDecryptData(buf, index, len,
        mSession.mDeviceKey, mMsgCounter[DEVICE_MSG_COUNTER],
        true, mRetVal, false);
  ...
  private void sendSuccessResponseForSessionTermination(APDU apdu) {
    ...
    //"status" : uint
    mEncoder.encodeRawData(mMdlSpecifications.status,
        (short) 0, (short) mMdlSpecifications.status.length);
  ...
  private short processDeviceRequest(
    ...
    if (Util.arrayCompare(
        buf, mStructure[0], mMdlSpecifications.DEVICE_REQ_VERSION, (short) 0, mStructure[1]) != 0) {
      ISOException.throwIt(ISO7816.SW_DATA_INVALID);
    }
   ...
   *
   * DeviceSigned = { "nameSpaces" : DeviceNameSpacesBytes, ; Returned data elements "deviceAuth" :
  ...
  short addEnvAndSessionHeaders(byte[] buf, short index, short len,
      short respLength) {
    ...
    mEncoder.startMap((short) 1);
    //"data" : bstr
    mEncoder.encodeRawData(mMdlSpecifications.session_data,
        (short) 0, (short) mMdlSpecifications.session_data.length);
  ...
  private void processGetResponse(APDU apdu, boolean firstMessage) {
    ...
    if (firstMessage) {
      ...
      mSession.beginIncrementalEncryption(mMsgCounter[DEVICE_MSG_COUNTER], mScratchPad, (short) 0,
    ...
    while (index < mContext.mChunkSize[0]) {
      ...
      if (mContext.mBufReadIndex[0] < mContext.mBufWriteIndex[0]) {
        ...
        index += len;
      } else if (mContext.mRemainingBytes[0]
          > 0) { // There is remaining data required to be encoded.
        ...
        mContext.mBufWriteIndex[0] += mSession.readAndClearBufferedData(mContext.mBuffer,
            mContext.mBufWriteIndex[0]);
        mEncoder.init(
            mContext.mBuffer, mContext.mBufWriteIndex[0],
        ...
        mContext.mRemainingBytes[0] = (short) (mContext.mRemainingBytes[0]
            - SEProvider.AES_GCM_TAG_LENGTH);
   ...
   */
  public boolean validateChain(MdocPresentationPkg doc,
    ...
    while (i > 2) {
      ...
      mX509CertHandler.decodeCert(buf, certStart, certEnd, mRetVal,
      ...
      if (count > 1 && attKeyLen > 0 &&
          !mSEProvider.validateEcDsaSign(buf,
  ...
  private boolean processItemsRequest(byte[] buf, short itemsBytesStart, short itemsBytesLen,
    ...
    if (doc != null) {
      ...
      if (processReaderAuth(req, doc, mStructure, buf, itemsBytesStart, itemsBytesLen,
          readerAuthStart, readerAuthLen, mScratchPad, (short) 0, (short) mScratchPad.length)) {
        return req.init(doc, mSession, buf, nameSpacesStart, nameSpacesLen,
            mScratchPad, (short) 0, (short) mScratchPad.length);
      }
    }
    return false;

...
public class MdocPresentationPkg {
     ...
     * document is persistently stored by provisioning applet and based on the usage counter it gets
     * swapped in and out. When reader queries about the data elements of the presentation package
     * then presentation applet generates a view of this document, which gets presented to the
     * reader. In order to quickly create the presentation view, this class maintains several
     * indexes such as namespace table, elements table, etc.
     */
    ...
    public short getDataStart() { return 0; }

    public short getDataLength() { return mDataEnd; }

    public byte[] getBuffer() { return mHeap; }
    ...
    public void resetUsageCount() { mUsageCount = 0; }

    public short getUsageCount() { return mUsageCount; }

    public KeyPair getAuthKeyPair() { return mAuthKey; }
    ...
    public short findElementEntry(short nsIndex, byte[] buf, short start, short len) {
        ...
        for (short i = elemStart; i < elemEnd; i += ELEM_TABLE_ROW_SIZE) {
            if (Util.arrayCompare(mHeap, mElementTable[(short)(i + ELEM_KEY_ID_OFFSET)], buf, start,
                                  len) == 0) {
                return i;
            }
        }
    ...
    public short read(byte[] buf, short start, short len, short ns, short elem, short elemDataPtr) {
        ...
        short remainingBytes =
            (short)(mElementTable[(short)(elem + ELEM_START_OFFSET)] +
    ...
    public void enumerate(short[] temp) {
        ...
        for (byte i = 0; i < mNsTableSize; i += NS_TABLE_ROW_SIZE) {
            ...
            while (items > 0) {
                ...
                mMdlSpecifications.decodeTaggedStructure(struct, temp, mHeap, start,
                                                         (short)(end - start));
    ...
    public short getIssuerAuthStart() { return mIssuerAuthStart; }

    public short getIssuerAuthLength() { return mIssuerAuthLength; }

    public void create(short size) { allocMem(size); }

    public void delete() {
        mHeapIndex = 0;
        mDigestMappingStart = mDocTypeStart = mIssuerAuthStart = mReaderAccessKeysStart =
            mReaderAccessKeysLen = mDigestMappingLength = mIssuerAuthLength = mDocTypeLen = -1;
        mElementTableSize = mNsTableSize = 0;
        freeMem();
    }

    public void setPreAllocated() { mPreAllocatedMem = true; }

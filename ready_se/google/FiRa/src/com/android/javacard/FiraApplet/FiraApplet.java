...
//    case FIRASpecs.INS_STORE_DATA:
    ...
    private short pushDispatchResponse(byte[] buf, short index, short len, short status,
        ...
        // Note: There can be up to 8 tags in a response and if we consider 4 bytes per tag then
    ...
    // the command in processTunnel. So on initiator this method is called to handle the response
    // of tunnel i.e. TUNNEL_ACTIVE state. On the responder side it should be idle state.
    ...
    private short getPutCmdRouteInfo(byte[] buf, short index, short len,
        ...
        if (routeTarget != FiraSpecs.VAL_SERVICE_APPLET) {
            // Currently, routing to application is not supported, but can be easily
            // supported in future.
            ISOException.throwIt(ISO7816.SW_INS_NOT_SUPPORTED);
        } else {
    ...
    private short routeToServiceApplet(byte[] buf, short tagStart, short tagLen, byte[] outBuf,
        ...
        // Check whether there is data and it is not zero length
        FiraUtil.getTag(FiraSpecs.TAG_CMD_ROUTING_DATA, buf, tagStart, tagLen, false, sRetValues);

        // At least two tags with at least one byte of value must be present
        if (sRetValues[2] < 6) {
            ISOException.throwIt(ISO7816.SW_WRONG_DATA);
        }
        ...
        if (ref == FiraSpecs.INVALID_VALUE) {
            ...
            // and that matches the registered list of Service Applets
            ...
            // shareable interface this will copy the memory.
    ...
    private short processImportADFCmd(byte[] buf, short inputStart, short inputLen,
        ...
        // using AES block cipher with padding. Padding may add upto one block of padding bytes
        // (16 bytes) If not then use the cache buf.
    ...
    private short processSwapADFCmd(byte[] buf, short inputStart, short inputLen,
        ...
        if (buf[ISO7816.OFFSET_P1] == FiraSpecs.INS_P1_SWAP_ADF_OP_ACQUIRE) {
            ...
            try {
                ...
                // free the slot in case of the exception
    ...
    // buf as apdu because it handles also P1 and P2. So care must be taken that complete apdu
    // buf is passed to this method.
    ...
    private short processDeleteAdfCmd(byte[] buf, short inputStart, short inputLen,
        ...
        if (slotId != FiraSpecs.INVALID_VALUE && slotId != FiraRepository.ROOT_SLOT
            ...
            // No slot/ADF selected - OID must be given
    ...
    private short processCreateAdfCmd(byte[] buf, short inputStart, short inputLen,
        ...
        // in slot specific way.
        if (context.getSlot() != FiraRepository.ROOT_SLOT) {
            ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
        }

        // Reserve a static slot. Note: we do not set the slot in context because there
        // can be multiple create adfs for each static slot. Before manage adf there will
        // be select adf.

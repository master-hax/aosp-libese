...
public class Scp11Lib {
    ...
    private void InitStaticFields() {
        ...
        if (sInData == null) {
            ...
            sCertificate = Certificates.getInstance();
        }
    }
    ...
    //  The concatenation of the following values shall be used for SharedInfo as input for the Key
    //  Derivation process:
    ...
    private short deriveSessionKey(byte keyUsageQual, byte keyType, byte keyLength,
        ...
        short ShSLength = 0;
        short skSdEckaSize =
                ClientContext.getSkSdEcka(kVn, mKeySetBuff, (short) 0, mFiraClientContext);
        ...
        ShSssSize = mCrypto.generateSecretEC_SVDP_DHC(mKeySetBuff, (short)0, skSdEckaSize,
                sCertificate.getOcePublicKey(), (short)0, sCertificate.getOcePublicKeySize(),
                sInData, ShSesSize);
        ...
        if (hostCardIDLength > (short)0) {
            Util.arrayCopyNonAtomic(hostCardID, hostCardIDOffset, sInData, (short) (ShSLength + 3),
                    hostCardIDLength);
        }
    ...
    private short generateReceiptAndResponse(byte[] buffer, short bufferOffset, short tagA6Offset,
            short tagA6Length, short tag5f49Offset, short tag5f49Length, byte keyUsageQual,
            byte keyType, byte keyLength, byte[] hostCardID, short hostCardIDOffset,
            short hostCardIDLength, byte kId, byte kVn, byte[] sendOutBuff,
            short sendOutBuffOffset) {
        ...
        byteCnt = BerTlvBuilder.fillLength(sendOutBuff, pkSdEckaSize, (short)
                (sendOutBuffOffset + index));
        index += byteCnt;
        ...
        mCrypto.genCmacAes128(sOutData, (short) 0, (short) keyLength, sInData, inputDataOffset,
                inputDataLength, sendOutBuff, (short) (sendOutBuffOffset + index));
    ...
    public short parseAndVerifyCrtGenerateReceipt(byte[] buffer, short bufferOffset, short bufferLen,
        ...
        if (ClientContext.getSkSdEcka(kVn, mKeySetBuff, (short) 0, mFiraClientContext)
                == (short) 0) {
        ...
        while (index < bufferLen) {
            ...
            index += tagByteCnt;
            dataLenByteCnt = BerTlvParser.getTotalLengthBytesCount(buffer,
    ...
    public boolean parseAndVerifySignature(byte[] buffer, short offset, short length,
        ...
        while (index < length) {
            ...
            // NOTE :-
            //  1) in-case of certificate chaining immediate PK will be stored in 'mPkOceEcka' itself
            //    which will be used as a verifier key for next certificate. (at the end
            //    'mPkOceEcka' is a final key, if '00 80': Key agreement is present)
            //  2) GPC_2.3_F_SCP11_v1.2.1 section 6.4.1
            //    - If no whitelist is defined for the referenced PK.CA-KLOC.ECDSA
            //     (see section 6.8), CSNs found in the certificate chain shall not be checked.
            //     Otherwise, if the CSN of the first (or only) CERT.KA-KLOC.ECDSA is not
            //     referenced in the whitelist, then the certificate (and the entire certificate
            //     chain) shall be rejected. Other CSNs found in the certificate chain shall not
            //     be checked (i.e. other certificates following in the chain are not required to
            //     be referenced in this whitelist in order to be accepted).
            ...
            if (!sCertificate.verifyCert(buffer, (short) (offset + index), dataLen,
                    firstCert/*for CSN check*/) ||
                    !mCrypto.verifyECDSAPlainSignatureSha256(
                            firstCert ? mKeySetBuff : sCertificate.getOcePublicKey(), (short) 0,
                            firstCert ? pkCaKlocEcdsaSize : sCertificate.getOcePublicKeySize(), buffer,
                            (short) (offset + index), sCertificate.getSignatureOffset(), buffer,
                            (short) (offset + sCertificate.getSignatureDataOffset() + index),
                            sCertificate.getSignatureLength())) {
                ISOException.throwIt(CERT_VERIFICATION_FAILED);
            }

            // if CERT.OCE.ECKA ('00 80': Key agreement) found return success
            if (sCertificate.getKeyUsageStatus() == (byte) 0x80) {
                return true;
            }

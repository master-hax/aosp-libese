...
public class Scp3Lib {
    ...
    public void setKeys(byte[] keyBuff, short keyBuffOffset, byte keyLen) {
        Util.arrayCopyNonAtomic(keyBuff, (short) keyBuffOffset, mS_ENC, (short) 0,
                (short) keyLen);
        Util.arrayCopyNonAtomic(keyBuff, (short) (keyBuffOffset + keyLen), mS_MAC, (short) 0,
                (short) keyLen);
        Util.arrayCopyNonAtomic(keyBuff, (short) (keyBuffOffset + (keyLen * 2)), mS_RMAC,
                (short) 0, (short) keyLen);
        Util.arrayCopyNonAtomic(keyBuff, (short) (keyBuffOffset + (keyLen * 3)), mS_DEK,
                (short) 0, (short) keyLen);
    ...
    private short encryptIV(boolean resApdu) {
        ...
        return mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mS_ENC, (short) 0,
                sNullBytes16, (short) 0, BLOCK16, resApdu ? mEncryptionCounterResponse :
                    mEncryptionCounter, (short) 0, (short) 16, sOutData, (short) 0);
    }

    private short decryptData(byte securityLevel, byte[] buff, short buffOffset, short buffLen,
            boolean resApdu) {
        short outLen = 0;
        short cdOffset = (short) (buff[(short) (buffOffset + ISO7816.OFFSET_CDATA)] ==
                (byte) 0x00 ? 7 : 5);
    ...
    private short wrapResponse(byte securityLevel, byte[] buff, short buffOffset, short buffLen) {
        ...
        // First: Encrypt data if buffLen > 2 & R_ENCRYPTION based on
        // 'GPC_2.3_D_SCP03_v1.1.2_PublicRelease.pdf' figure 6-5
        if (((securityLevel & R_ENCRYPTION) == R_ENCRYPTION) && (buffLen > 2 /*status*/)) {
            ...
            short totalLenWithpad = pad80(sInData, (short) 0, (short) (buffLen - 2), BLOCK16);
            outOffset = outLen;
            outLen = mCrypto.genAes128CbcNopadOutput(Cipher.MODE_ENCRYPT, mS_ENC, (short) 0,
        ...
        if ((securityLevel & R_MAC) == R_MAC && (buffLen > 2 /*status*/)) {
            // Second: calculate MAC
            if (cipheredData) {
                Util.arrayCopyNonAtomic(sOutData, outOffset, sInData, (short) 0, outLen);
            } else {
                Util.arrayCopyNonAtomic(buff, buffOffset, sInData, (short) 0, buffLen);
                outLen = buffLen;
            }
            ...
            outLen -= 2;
            Util.arrayCopyNonAtomic(sInData, (short) 0, buff, buffOffset, outLen);
            // The Secure channel shall support a MAC of 8 bytes length
            // (even if the AES block length is 16 bytes). Hence the eight most significant
    ...
    /**
     * key derivation function based on scp03 protocol (SCP03_v1.1.2_PublicRelease) and
     * fiRa (CSML - 7.5.3.3.1) based on argument 'uwbSessionKeyFlag'
     *

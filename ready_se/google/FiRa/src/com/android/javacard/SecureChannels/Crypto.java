...
public class Crypto {
    ...
    private static KeyAgreement sKeyAgreement;
    private static Signature sMacSignature128;
    private static Signature sSignerEcdsaPlain;
    private static Cipher sAesCipher;
    private static Cipher sAesCipherT;

    private static byte[] mInputData;
    private static Crypto sCrypto;

    private Crypto() {
        sEcKeyPair = new KeyPair(KeyPair.ALG_EC_FP, KeyBuilder.LENGTH_EC_FP_256);
        initStatics();
        initECKey(sEcKeyPair);
        mInputData = JCSystem.makeTransientByteArray((short) 512, JCSystem.CLEAR_ON_RESET);
        sKeyAgreement = KeyAgreement.getInstance(KeyAgreement.ALG_EC_SVDP_DHC, false);
        sMacSignature128 = Signature.getInstance(Signature.ALG_AES_CMAC_128, false);
        sSignerEcdsaPlain = Signature.getInstance(MessageDigest.ALG_SHA_256,
                Signature.SIG_CIPHER_ECDSA_PLAIN, Cipher.PAD_NULL, false);
        sAesCipher = (Cipher) Cipher.getInstance(Cipher.ALG_AES_BLOCK_128_CBC_NOPAD, false);
        sAesCipherT = (Cipher) Cipher.getInstance(Cipher.ALG_AES_CBC_ISO9797_M2, false);
    }
    ...
    protected boolean verifyECDSAPlainSignatureSha256(byte[] pubKey, short pubKeyOffset,
        ...
        ECPublicKey key = (ECPublicKey) sEcKeyPair.getPublic();
        key.setW(pubKey, pubKeyOffset, pubKeyLen);
        sSignerEcdsaPlain.init(key, Signature.MODE_VERIFY);
        return sSignerEcdsaPlain.verify(inputDataBuf, inputDataStart, inputDataLength, sigBuffer,
                sigOffset, sigLength);
    }

    protected short ecdSAPlainSignatureSha256(byte[] priKey, short priKeyOffset, short priKeyLen,
            byte[] inputDataBuf, short inputDataStart, short inputDataLength, byte[] sigBuffer,
            short sigOffset) {
        ECPrivateKey key = (ECPrivateKey) sEcKeyPair.getPrivate();
        key.setS(priKey, priKeyOffset, priKeyLen);
        sSignerEcdsaPlain.init(key, Signature.MODE_SIGN);
        return sSignerEcdsaPlain.sign(inputDataBuf, inputDataStart, inputDataLength, sigBuffer,
                sigOffset);
    }

    protected short generateSecretEC_SVDP_DHC(byte[] priKey, short priKeyOffset, short priKeySize,
            byte[] pubKey, short pubKeyOffset, short pubKeySize, byte[] secret,
            short secretOffset) {
        ECPrivateKey ecPrivKey = (ECPrivateKey) sEcKeyPair.getPrivate();
        ecPrivKey.setS(priKey, priKeyOffset, priKeySize);

        sKeyAgreement.init(ecPrivKey);
        return sKeyAgreement.generateSecret(pubKey, pubKeyOffset, pubKeySize, secret, secretOffset);
    }

    // X9.63 Key Derivation Function
    protected short kdfX963(byte[] shSbuff, short shSOffset, short shSLength, byte[] sharedInfoBuff,
    ...
    protected short genCmacAes128(byte[] key, short keyOffset, short keyLength, byte[] inputData,
            short inputDataOffset, short inputDataLength, byte[] out, short outOffset) {
        sMacSignature128.init(setAESkey(key, keyOffset, keyLength), Signature.MODE_SIGN);
        return sMacSignature128.sign(inputData, inputDataOffset, inputDataLength, out, outOffset);
    }
    ...
    protected short genCmacAes128(byte[] key, short keyOffset, short keyLength, byte[] inputData,
        ...
        sMacSignature128.init(setAESkey(key, keyOffset, keyLength), Signature.MODE_SIGN);
        sMacSignature128.update(chainingValue, chainingValueOffset, (short) 16);
        return sMacSignature128.sign(inputData, inputDataOffset, inputDataLength, out, outOffset);
    }

    protected boolean verifyCmacAes128(byte[] key, short keyOffset, short keyLength, byte[] inputData,
            short inputDataOffset, short inputDataLength, byte[] sigBuffer, short sigOffset,
            short sigLength) {

        if (sMacSignature128 == null) {
            sMacSignature128 = Signature.getInstance(Signature.ALG_AES_CMAC_128, false);
        }

        sMacSignature128.init(setAESkey(key, keyOffset, keyLength), Signature.MODE_VERIFY);

        return sMacSignature128.verify(inputData, inputDataOffset, inputDataLength, sigBuffer,
                sigOffset, sigLength);
    }

    // Padding Method 2
    protected short addPaddingM2(byte[] buffer, short offset, short length) {
    ...
    protected short unpadM2(byte[] buffer, short bufferOffset, short bufferLength) {
    ...
    protected short genAes128CbcNopadOutput(byte mode, byte[] keyAes, short keyAesOffset, byte[] iv,
        ...
        sAesCipher.init(key, mode, iv, ivOffset, ivLength);
        // encrypt/decrypt
        return sAesCipher.doFinal(inData, inDataOffset, inDataLength, outData, outDataOffset);
    }

    protected short genAes128CbcOutput(byte mode, byte[] keyAes, short keyAesOffset, byte[] iv,
            short ivOffset, short ivLength, byte[] inData, short inDataOffset, short inDataLength,
            byte[] outData, short outDataOffset) {
        ...
        sAesCipherT.init(key, mode, iv, ivOffset, ivLength);

        // encrypt/decrypt
        return sAesCipherT.doFinal(inData, inDataOffset, inDataLength, outData, outDataOffset);
    }

    // NOTE: Change/merge the following functions
    ...
    protected short cmacKdfCounterMode(byte[] key, short keyOffset, byte[] label, short labelStart,
        ...
        AESKey aeskey = (AESKey) KeyBuilder.buildKey(KeyBuilder.TYPE_AES, KeyBuilder.LENGTH_AES_128,
                false);
        aeskey.setKey(key, (short) keyOffset);
        sMacSignature128.init(aeskey, Signature.MODE_SIGN);

        // ZERO byte (1 byte separation) 
        mInputData[0] = 0x00;

        while (i <= counter) {
            sMacSignature128.update(label, labelStart, (short) labelLen);  // label
            sMacSignature128.update(mInputData, (short) 0, (short) 1);     // 1 byte separation
                                                                           // indicator
            sMacSignature128.update(L, Loffset, Llength); // L

            mInputData[1] = i;
            sMacSignature128.update(mInputData, (short) 1, (short) 1);      // counter
            sMacSignature128.update(context, contextStart, contextLength);  // context

            // signature of 16 bytes
            pos = sMacSignature128.sign(L, (short) Loffset, (short) Llength, outData,
    ...
    protected short cmacKdfCounterModeFiRa2(byte[] key, short keyOffset, byte label1, byte label2,
            byte[] outData, short outDataOffset) {
        ...
        // long as it is unambiguously defined.
        ...
        return sMacSignature128.sign(mInputData, (short) 0, inLen, outData, outDataOffset);
    }

    protected short cmacKdfCounterModeUWBsessionKey(byte[] key, short keyOffset, short keyLength,
        ...
        while (i <= counter) {
            mInputData[3] = i;
            sMacSignature128.update(mInputData, (short) 0, (short) 4);    // counter
            sMacSignature128.update(label, labelStart, (short) labelLen); // label

            if (diversificationLength >= (keyLength == (short) 16 ? (short) 4 : (short) 20)) {
                // Diversification Data
                sMacSignature128.update(diversification, diversificationOffset,
                        diversificationLength);
            } else {
                sMacSignature128.update(mInputData, (short) 8,
            ...
            // signature of 16 bytes
            pos = sMacSignature128.sign(mInputData, (short) 4, (short) 4, outData,

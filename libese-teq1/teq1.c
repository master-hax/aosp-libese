...
int teq1_transmit(struct EseInterface *ese, struct Teq1Frame *frame) {
   ...
   */
  teq1_trace_transmit(frame->header.PCB.val);
...
int teq1_receive(struct EseInterface *ese, float timeout,
  ...
  teq1_trace_receive(frame->header.PCB.val);
...
uint8_t teq1_frame_error_check(struct Teq1Frame *frame) {
  uint8_t lrc = 0;
  if (frame->header.PCB.val == 255)
    return R(0, 1, 0); /* Other error */

  lrc = teq1_compute_LRC(frame);
  if (frame->INF[frame->header.LEN] != lrc) {
    ALOGV("Invalid LRC %x instead of %x", frame->INF[frame->header.LEN], lrc);
    return R(0, 0, 1); /* Parity error */
  }

  switch (frame->header.PCB.type) {
  case kPcbTypeSupervisory:
    if (frame->header.PCB.val != S(RESYNC, RESPONSE) && frame->header.LEN != 1)
      return R(0, 1, 0);
    break;
  case kPcbTypeReceiveReady:
...
enum RuleResult teq1_rules(struct Teq1State *state, struct Teq1Frame *tx_frame,
  ...
  while (1) {
    ...
    if ((R_err.val = teq1_frame_error_check(rx_frame)) != 0) {
    ...
    case TEQ1_RULE(I(1, 0), I(0, 0)):
      ...
      /* After chaining I(0, 1) -> R(1) -> I(1, 0) the last
       * response is I(0, 0), instead of a matching I(1, 0)
       */
      ALOGV("Workaround NXP:I(1,0) -> I(0, 0)");
      /* Read app data & return. */
      teq1_get_app_data(state, rx_frame);
      return kRuleResultComplete;
    ...
    case TEQ1_RULE(I(0, 0), I(0, 1)):
      /* Prep R(1) */
      teq1_get_app_data(state, rx_frame);
      next_tx->header.PCB.val = R(1, 0, 0);
    ...
    case TEQ1_RULE(R(1, 0, 0), I(1, 0)):
      /* Read the data and return kRuleResultComplete. */
      teq1_get_app_data(state, rx_frame);
      return kRuleResultComplete;
    case TEQ1_RULE(R(0, 0, 0), I(0, 1)):
      /* Read the a partial data and send R(1) */
      teq1_get_app_data(state, rx_frame);
      next_tx->header.PCB.val = R(1, 0, 0);
      return kRuleResultContinue;
...
API size_t teq1_transceive(struct EseInterface *ese,
  ...
  int was_reset = 0;
  struct Teq1State init_state = TEQ1_INIT_STATE(tx_buf, tx_len, rx_buf, rx_len);
  struct Teq1State state = TEQ1_INIT_STATE(tx_buf, tx_len, rx_buf, rx_len);
  int active = 0;
  int done = 0;
  enum RuleResult result = kRuleResultComplete;
  const struct Teq1ProtocolOptions *opts = ese->ops->opts;

  /* First I-block is always I(0, M). */

...
TEST_F(Teq1FrameErrorCheck, info_parity) {
  ...
  while (*pcb != 255) {
    frame_.header.PCB.val = *pcb;
    frame_.header.LEN = 2;
    frame_.INF[0] = 'A';
    frame_.INF[1] = 'B';
    frame_.INF[2] = teq1_compute_LRC(&frame_);
    EXPECT_EQ(0, teq1_frame_error_check(&frame_));
    frame_.INF[2] = teq1_compute_LRC(&frame_) - 1;
    EXPECT_EQ(TEQ1_R(0, 0, 1), teq1_frame_error_check(&frame_));
    pcb++;
  }
};
...
class Teq1CompleteTest : public Teq1ErrorFreeTest {
 public:
  ...
  virtual void RunRules() {
    teq1_trace_header();
    teq1_trace_transmit(tx_frame_.header.PCB.val);
    teq1_trace_receive(rx_frame_.header.PCB.val);
...
TEST_F(Teq1CompleteTest, I10_I10_data) {
  ...
  rx_frame_.header.PCB.val = TEQ1_I(1, 0);
  rx_frame_.INF[INF_LEN] = teq1_compute_LRC(&rx_frame_);
  RunRules();
  // Ensure that the rx_frame data was copied out to rx_data.
...
TEST_F(Teq1ErrorFreeTest, I00_WTX0_WTX1_data) {
  ...
  enum RuleResult result = teq1_rules(&state_,  &tx_frame_, &rx_frame_, &tx_next_);
  teq1_trace_transmit(tx_next_.header.PCB.val);

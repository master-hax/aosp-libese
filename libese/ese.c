...
API int ese_transceive(struct EseInterface *ese, const uint8_t *tx_buf,
  ...
  const struct EseSgBuffer tx = {
      .base = (uint8_t *)tx_buf, .len = tx_len,
  };
...
API int ese_transceive_sg(struct EseInterface *ese,
                          const struct EseSgBuffer *tx_bufs, uint32_t tx_segs,
                          struct EseSgBuffer *rx_bufs, uint32_t rx_segs) {
  uint32_t recvd = 0;
  if (!ese) {
    return -1;
  }

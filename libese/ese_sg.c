...
API uint32_t ese_sg_length(const struct EseSgBuffer *bufs, uint32_t cnt) {
...
API uint32_t ese_sg_to_buf(const struct EseSgBuffer *src, uint32_t src_cnt,
                           uint32_t start_at, uint32_t length, uint8_t *dst) {
  ...
  while (remaining && buf < src + src_cnt) {
    ...
    ese_memcpy(dst, buf->base + start_at, copy_len);
    dst += copy_len;
    remaining -= copy_len;
    start_at = 0;
    buf++;
...
API uint32_t ese_sg_from_buf(struct EseSgBuffer *dst, uint32_t dst_cnt,

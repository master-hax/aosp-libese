...
class CoreSlots implements Slots {
    static final byte NUM_SLOTS = 16;
    ...
    @Override
    public short read(short rawSlotId, byte[] key, short keyOffset,
    ...
    private static class Slot {
         ...
         * @return status code
         */
        public short read(byte[] keyBuffer, short keyOffset, byte[] outBuffer, short outOffset) {
        ...
        private static void throttle(byte[] bArray, short bOff, short failureCount) {
            ...
            if (failureCount == 0) {
                // 0s
                Util.arrayFillNonAtomic(bArray, bOff, (short) 4, (byte) 0);
            } else if (failureCount > 0 && failureCount <= 10) {
                if (failureCount % 5 == 0) {
                    // 30s
                    Util.setShort(bArray, bOff, (short) 0);
                    Util.setShort(bArray, (short) (bOff + 2), thritySecondsInMilliseconds);
                }  else {
                    // 0s
                    Util.arrayFillNonAtomic(bArray, bOff, (short) 4, (byte) 0);
                }
                ...
                final short highBytes = (short) (thritySecondsInMilliseconds >> (16 - shift));
                final short lowBytes = (short) (thritySecondsInMilliseconds << shift);
                Util.setShort(bArray, bOff, highBytes);
                Util.setShort(bArray, (short) (bOff + 2), lowBytes);
            } else {
                // 1 day in ms = 1000 * 60 * 60 * 24 = 0x526 5C00
                final short highBytes = 0x0526;
                final short lowBytes = 0x5c00;
                Util.setShort(bArray, bOff, highBytes);
                Util.setShort(bArray, (short) (bOff + 2), lowBytes);
            }
        }

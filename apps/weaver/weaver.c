...
const uint8_t kWrite[] = {0x80, 0x04, 0x00, 0x00}; // slotid + key + value
const uint8_t kRead[] = {0x80, 0x06, 0x00, 0x00};  // slotid + key + value
...
ESE_API EseAppResult ese_weaver_read(struct EseWeaverSession *session,
  ...
  // Send the command
  const int rx_len = ese_transceive_sg(session->ese, tx, 4, rx, 2);
  ...
  // wrong key
  if (rx_buf[rx_len - 1] == 0x6a && rx_buf[rx_len - 2] == 0x85) {
    ALOGI("ese_weaver_read wrong key provided");
    *timeout = get_uint32(rx_buf);
    return ESE_WEAVER_READ_WRONG_KEY;
  }
  // backoff
  if (rx_buf[rx_len - 1] == 0x6a && rx_buf[rx_len - 2] == 0x87) {
    ALOGI("ese_weaver_read wrong key provided");
    *timeout = get_uint32(rx_buf);

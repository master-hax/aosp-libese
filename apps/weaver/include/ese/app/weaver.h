...
const EseAppResult ESE_WEAVER_READ_WRONG_KEY = ese_make_app_result(0x6a, 0x85);
const EseAppResult ESE_WEAVER_READ_TIMEOUT = ese_make_app_result(0x6a, 0x87);
...
EseAppResult ese_weaver_read(struct EseWeaverSession *session, uint32_t slotId,
                             uint8_t *key, uint8_t *value, uint32_t *timeout);

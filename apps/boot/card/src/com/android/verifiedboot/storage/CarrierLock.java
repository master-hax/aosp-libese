...
class CarrierLock implements LockInterface {
     ...
     * {@inheritDoc}
     *
     * Returns 0x0001 if {@link #initialize()} has not yet been called.
     */
    @Override
    ...
    private short verifyUnlock(byte[] deviceData, short deviceDataOffset,
        ...
        if (unlockToken.length < (short)(1 + NONCE_SIZE + PK_MOD.length)) {
            return 0x0002;
        }
        // Only supported version is 0x01.
        if (unlockToken[0] != 0x01) {
            return 0x0003;
        }
        ...
        Util.arrayCopy(unlockToken, (short)(unlockTokenOffset + 1),
        ...
        if (verifier.verify(
                ...
                (short)(unlockTokenOffset + 1 + NONCE_SIZE),
                (short)(unlockTokenLength - (1 + NONCE_SIZE))) ==
                false) {
            return 0x0004;
        }
        short i = (short) NONCE_SIZE;
        while (i > 0) {
           if (lastNonce[(short)(lastNonceOffset + i)] >
               unlockToken[(short)(unlockTokenOffset + 1 + i)]) {
               return 0x0005;
           }
           if (lastNonce[(short)(lastNonceOffset + i)] <
               unlockToken[(short)(unlockTokenOffset + 1 + i)]) {
               // We can unlock. The nonce _must_ be larger than
               // the last nonce.
               return 0;
           }
           // Only proceed if the current bytes are equal to
           // use the next byte as the tie breaker.
           i -= 1;
        }
        return 0x0006;
    }
     ...
     *   fakeLastNonce | fakeDeviceData | version | testNonce | signature
     *
    ...
    public short testVector(byte[] buffer, short offset, short length) {
        return verifyUnlock(buffer, offset,  // fake last nonce.
                            buffer, (short)(offset + NONCE_SIZE), // device data
                            // unlock data

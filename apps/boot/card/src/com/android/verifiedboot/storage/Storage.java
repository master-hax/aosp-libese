...
public class Storage extends Applet implements ExtendedLength, Shareable {
     ...
     */
    private static GlobalStateImpl globalState;
     ...
     */
    private static byte[] lockStorage;
    private static LockInterface[] locks;
    // Indices into locks[].
    ...
    public static void install(byte[] bArray, short bOffset, byte bLength) {
        ...
        // In order by precedence over the next.
        locks[LOCK_CARRIER] = new CarrierLock();
        locks[LOCK_DEVICE] = new BasicLock(false, false, (short) 0);
        locks[LOCK_BOOT] = new BasicLock(true, false, (short) 0);
        locks[LOCK_OWNER] = new BasicLock(true, true, (short) 1024);
        // locks[3] = new OwnerLock();
        ...
        new Storage().register(bArray, (short) (bOffset + 1),
            bArray[bOffset]);
    }
    ...
    private short enforceLockPolicy(byte p1) {
        // Ensure all controlling locks are unlocked to allow toggling.
        //
        // E.g., OWNER_LOCK can be toggled as long as BOOT_LOCK == 0.
        // BOOT_LOCK can be toggled as long as DEVICE_LOCK == 0.
        // CARRIER_LOCK is the parent.
        //
        // We cannot allow a transition of a child lock to a locked state if
        // the parent lock is locked because it may have side effects -- e.g.,
        // with the OWNER_LOCK during a BOOT_LOCK.
        //
        byte l = (byte) 0;
        ...
        for ( ; l < (byte) locks.length; ++l) {
            if (p1 == l) {
                return 0x0;
            }
            resp = locks[l].get(temp, (short) 0);

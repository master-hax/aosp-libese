...
// XXX Add LSBackup to the list once it is linkable.
public class Storage extends Applet implements ExtendedLength, Shareable {
    ...
    private Storage() {
        ...
        ((BasicLock)locks[LOCK_BOOT]).addRequiredLock(locks[LOCK_CARRIER]);
        ((BasicLock)locks[LOCK_BOOT]).addRequiredLock(locks[LOCK_DEVICE]);
        ((BasicLock)locks[LOCK_BOOT]).requireBootloader(true);
        ...
        ((BasicLock)locks[LOCK_OWNER]).addRequiredLock(locks[LOCK_BOOT]);
        ((BasicLock)locks[LOCK_OWNER]).requireMetadata(true);
    ...
    public Shareable getShareableInterfaceObject(AID clientAid, byte arg) {
        if (clientAid != null) {  // TODO: check AID.
            switch (arg) {
                case (byte) 0:
                  return globalState;
                // XXX: case LSBackup.LS_BACKUP_PARAMETER:
                case (byte) 0x1:
                  return this;
            }
        }
    ...
    private static LSPullModeRestore getLSPullModeRestore() {
        try {
          return (LSPullModeRestore)JCSystem.getAppletShareableInterfaceObject(
            ...
            (byte)1);
            // XXX LSPullModeRestore.LS_PULL_MODE_RESTORE_PARAMETER);
        } catch (CardRuntimeException e) {
            return null;
        }
    }
    */
    ...
    /* Pending NXP
        ...
        short length = 2;
        if (restore.pullData(buffer, (short)(0), length) == false) {
            return false;
        }
        Util.getShort(buffer, (short)0);
        if (restore.pullData(buffer, (short)(0), length) == false) {
            return false;
        }
        if (length > lockStorage.length) {
            // Truncation seems okay-ish for now.  We could attempt to delete
            // and reallocation, but it is likely more useful to tag each Lock
            // entry and make sure they still match up.
            length = (short)lockStorage.length;
        }
        Util.arrayCopy(buffer, (short)(0), lockStorage, (short)(0), length);
    ...
    */
    ...
    public boolean backup(LSPushModeBackup backup, byte[] buffer) {
        short length = (short)(2 + lockStorage.length);
        if (backup.start(length) == false) {
            return false;
        }
        Util.setShort(buffer, (short)0, (short)lockStorage.length);
        Util.arrayCopy(lockStorage, (short)0,
                       buffer, (short)2,
                       (short)(lockStorage.length));
